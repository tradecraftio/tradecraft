...
bool PSTInputSignedAndVerified(const PartiallySignedTransaction pst, unsigned int input_index, const PrecomputedTransactionData* txdata)
{
    ...
    if (txdata) {
        return VerifyScript(input.final_script_sig, spent_output.out.scriptPubKey, &input.final_script_witness, STANDARD_SCRIPT_VERIFY_FLAGS, MutableTransactionSignatureChecker{&(*pst.tx), input_index, spent_output.out.nValue, spent_output.refheight, *txdata, MissingDataBehavior::FAIL});
    } else {
        return VerifyScript(input.final_script_sig, spent_output.out.scriptPubKey, &input.final_script_witness, STANDARD_SCRIPT_VERIFY_FLAGS, MutableTransactionSignatureChecker{&(*pst.tx), input_index, spent_output.out.nValue, spent_output.refheight, MissingDataBehavior::FAIL});
    }
}
...
bool CombinePSTs(PartiallySignedTransaction& out, const std::vector<PartiallySignedTransaction>& pstxs)
{
