...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl& coinControl)
{
...

    {
        ...
        const auto& res = m_wallet->createTransaction(vecSend, -1 /* refheight */, coinControl, !wallet().privateKeysDisabled() /* sign */, nChangePosRet, nFeeRequired);
...
bool WalletModel::bumpFee(uint256 hash, uint256& new_hash)
{
    ...
    auto confirmationDialog = new SendConfirmationDialog(tr("Confirm fee bump"), questionString, "", "", SEND_CONFIRM_DELAY, !m_wallet->privateKeysDisabled(), getOptionsModel()->getEnablePSFRControls(), nullptr);
    ...
    if (retval == QMessageBox::Save) {
        ...
        bool complete = false;
        const TransactionError err = wallet().fillPST(SIGHASH_ALL, false /* sign */, true /* bip32derivs */, nullptr, pstx, complete);
...
{
    return control && control->HasSelected() ? wallet().getAvailableBalance(atheight, *control) : getCachedBalance().balance;
}
