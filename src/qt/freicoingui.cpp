...
const std::string FreicoinGUI::DEFAULT_UIPLATFORM =
...
FreicoinGUI::FreicoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &FreicoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &FreicoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &FreicoinGUI::showModalOverlay);
...
void FreicoinGUI::createActions()
{
    ...
    connect(overviewAction, &QAction::triggered, this, &FreicoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &FreicoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &FreicoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    m_load_pst_action = new QAction(tr("&Load PST from file…"), this);
    m_load_pst_action->setStatusTip(tr("Load Partially Signed Freicoin Transaction"));
    m_load_pst_clipboard_action = new QAction(tr("Load PST from &clipboard…"), this);
    m_load_pst_clipboard_action->setStatusTip(tr("Load Partially Signed Freicoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a freicoin: URI"));
    ...
    connect(quitAction, &QAction::triggered, this, &FreicoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &FreicoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &FreicoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &FreicoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &FreicoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(m_load_pst_action, &QAction::triggered, [this]{ gotoLoadPST(); });
        connect(m_load_pst_clipboard_action, &QAction::triggered, [this]{ gotoLoadPST(true); });
        connect(verifyMessageAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
        connect(verifyMessageAction, &QAction::triggered, [this]{ gotoVerifyMessageTab(); });
        connect(usedSendingAddressesAction, &QAction::triggered, walletFrame, &WalletFrame::usedSendingAddresses);
        connect(usedReceivingAddressesAction, &QAction::triggered, walletFrame, &WalletFrame::usedReceivingAddresses);
        connect(openAction, &QAction::triggered, this, &FreicoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &FreicoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &FreicoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &FreicoinGUI::createWallet);
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_migrate_wallet_action, &QAction::triggered, [this] {
            auto activity = new MigrateWalletActivity(m_wallet_controller, this);
            connect(activity, &MigrateWalletActivity::migrated, this, &FreicoinGUI::setCurrentWallet);
            activity->migrate(walletFrame->currentWalletModel());
        });
        connect(m_mask_values_action, &QAction::toggled, this, &FreicoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &FreicoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &FreicoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &FreicoinGUI::showDebugWindow);
}
...
void FreicoinGUI::createMenuBar()
{
    ...
    if(walletFrame)
    {
        ...
        file->addAction(m_load_pst_action);
        file->addAction(m_load_pst_clipboard_action);
        file->addSeparator();
    }
...
void FreicoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &FreicoinGUI::setCurrentWalletBySelectorIndex);
...
void FreicoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &FreicoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &FreicoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &FreicoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &FreicoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void FreicoinGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void FreicoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &FreicoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &FreicoinGUI::removeWallet);
...
WalletController* FreicoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void FreicoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &FreicoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &FreicoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &FreicoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &FreicoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &FreicoinGUI::incomingTransaction);
    connect(this, &FreicoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void FreicoinGUI::removeWallet(WalletModel* walletModel)
{
...
void FreicoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void FreicoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void FreicoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void FreicoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void FreicoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &FreicoinGUI::toggleHidden);
...
void FreicoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void FreicoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void FreicoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void FreicoinGUI::openClicked()
{
...
void FreicoinGUI::gotoSendCoinsPage(QString addr)
{
...
void FreicoinGUI::gotoLoadPST(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPST(from_clipboard);
}
#endif // ENABLE_WALLET

void FreicoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Freicoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void FreicoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void FreicoinGUI::updateHeadersSyncProgressLabel()
{
    int64_t headersTipTime = clientModel->getHeaderTipTime();
    int headersTipHeight = clientModel->getHeaderTipHeight();
    int estHeadersLeft = (GetTime() - headersTipTime) / Params().GetConsensus().aux_pow_target_spacing;
    if (estHeadersLeft > HEADER_HEIGHT_DELTA_SYNC)
        progressBarLabel->setText(tr("Syncing Headers (%1%)…").arg(QString::number(100.0 / (headersTipHeight+estHeadersLeft)*headersTipHeight, 'f', 1)));
}

void FreicoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void FreicoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &FreicoinGUI::quitRequested);
...
void FreicoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void FreicoinGUI::createWallet()
{
    ...
    connect(activity, &CreateWalletActivity::created, this, &FreicoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void FreicoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void FreicoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &FreicoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &FreicoinGUI::show);
                e->ignore();
            }
        }
    }
...
void FreicoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void FreicoinGUI::incomingTransaction(const QString& date, FreicoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(FreicoinUnits::formatWithUnit(unit, amount, true));
...
bool FreicoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool FreicoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void FreicoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void FreicoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void FreicoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void FreicoinGUI::updateWindowTitle()
{
...
void FreicoinGUI::detectShutdown()
{
...
void FreicoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void FreicoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(FreicoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = FreicoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void FreicoinGUI::subscribeToCoreSignals()
{
...
void FreicoinGUI::unsubscribeFromCoreSignals()
{
...
bool FreicoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<FreicoinUnit> units = FreicoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const FreicoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, FreicoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const FreicoinUnit u : FreicoinUnits::availableUnits()) {
        menu->addAction(FreicoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(FreicoinUnit newUnits)
{
    setText(FreicoinUnits::longName(newUnits));
}
