This directory contains the FreicoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Freicoin UI.
...
ClientModel is used by the main application `freicoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/bitcoin/bitcoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a freicoin:... link from a browser).
...
* UI elements like FreicoinAmountField, which inherit from QWidget.
* `freicoinstrings.cpp`: automatically generated
* `freicoinunits.(h/cpp)`: FRC / mFRC / etc handling
* `callback.h`
...
* don't change `local/freicoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "freicoin-qt" under "Run", which is where you can also add command line arguments)
