This directory contains the source code for the Freicoin graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a freicoin:... link from a browser).
...
**Note:** Do not change `local/freicoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `freicoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
