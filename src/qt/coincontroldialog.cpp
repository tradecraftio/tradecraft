    ...
    platformStyle(platformStyle)
{
    ...
    // see https://github.com/freicoin/freicoin/issues/5716
...
{
    GUIUtil::setClipboard(FreicoinUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = FreicoinUnits::FRC;
    ...
    l2->setText(FreicoinUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(FreicoinUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(FreicoinUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l6->setText(sPriorityLabel);                                             // Priority
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(FreicoinUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0 && (coinControl->nMinimumTotalFee < nPayFee))
    {
    ...
    toolTip1 += tr("This means a fee of at least %1 per kB is required.").arg(FreicoinUnits::formatHtmlWithUnit(nDisplayUnit, CWallet::GetRequiredFee(1000))) + "<br /><br />";
    toolTip1 += tr("Can vary +/- 1 byte per input.");

    QString toolTip2 = tr("Transactions with higher priority are more likely to get included into a block.") + "<br /><br />";
    toolTip2 += tr("This label turns red if the priority is smaller than \"medium\".") + "<br /><br />";
    toolTip2 += tr("This means a fee of at least %1 per kB is required.").arg(FreicoinUnits::formatHtmlWithUnit(nDisplayUnit, CWallet::GetRequiredFee(1000)));

    QString toolTip3 = tr("This label turns red if any recipient receives an amount smaller than %1.").arg(FreicoinUnits::formatHtmlWithUnit(nDisplayUnit, ::minRelayTxFee.GetFee(546)));

    // how many kria the estimated fee can vary per byte we guess wrong
    double dFeeVary;
    ...
    QString toolTip4 = tr("Can vary +/- %1 kria per input.").arg(dFeeVary);
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins) {
        ...
        BOOST_FOREACH(const COutput& out, coins.second) {
            ...
            {
                sAddress = QString::fromStdString(CFreicoinAddress(outputAddress).ToString());

                // if listMode or change => show freicoin address. In tree mode, address is not shown again for direct wallet address outputs
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, FreicoinUnits::format(nDisplayUnit, nSum));
