...
bool AddressTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        CTxDestination curAddress = CFreicoinAddress(rec->address.toStdString()).Get();
        if(index.column() == Label)
        {
            ...
            CTxDestination newAddress = CFreicoinAddress(value.toString().toStdString()).Get();
...
QString AddressTableModel::addRow(const QString &type, const QString &label, const QString &address)
{
    ...
    {
        LOCK(wallet->cs_wallet);
        wallet->SetAddressBook(CFreicoinAddress(strAddress).Get(), strLabel,
...
bool AddressTableModel::removeRows(int row, int count, const QModelIndex &parent)
{
    ...
    {
        LOCK(wallet->cs_wallet);
        wallet->DelAddressBook(CFreicoinAddress(rec->address.toStdString()).Get());
    }
    return true;
}
