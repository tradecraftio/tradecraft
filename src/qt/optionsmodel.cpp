...
/** Call node.updateRwSetting() with Freicoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Freicoin 22.x releases try to read these specific settings as
        ...
        // instead of strings, because freicoin 22.x will not try to read these.
...
//! Convert enabled/size values to freicoin -prune setting.
...
//! Get pruning enabled value to show in GUI from freicoin -prune setting.
...
//! Get pruning size value to show in GUI from freicoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayFreicoinUnit")) {
        settings.setValue("DisplayFreicoinUnit", QVariant::fromValue(FreicoinUnit::FRC));
    }
    QVariant unit = settings.value("DisplayFreicoinUnit");
    if (unit.canConvert<FreicoinUnit>()) {
        m_display_freicoin_unit = unit.value<FreicoinUnit>();
    } else {
        m_display_freicoin_unit = FreicoinUnit::FRC;
        settings.setValue("DisplayFreicoinUnit", QVariant::fromValue(m_display_freicoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if freicoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_freicoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<FreicoinUnit>() == m_display_freicoin_unit) return;
    m_display_freicoin_unit = new_unit.value<FreicoinUnit>();
    QSettings settings;
    settings.setValue("DisplayFreicoinUnit", QVariant::fromValue(m_display_freicoin_unit));
    Q_EMIT displayUnitChanged(m_display_freicoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    // (https://github.com/freicoin/gui/issues/567).
