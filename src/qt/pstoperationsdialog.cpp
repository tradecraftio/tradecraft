...
void PSTOperationsDialog::openWithPST(PartiallySignedTransaction pstx)
{
    ...
    if (m_wallet_model) {
        size_t n_could_sign;
        const auto err{m_wallet_model->wallet().fillPST(SIGHASH_ALL, /*sign=*/false, /*bip32derivs=*/true, &n_could_sign, m_transaction_data, complete)};
        if (err) {
            showStatus(tr("Failed to load transaction: %1")
                           .arg(QString::fromStdString(PSTErrorString(*err).translated)),
                       StatusLevel::ERR);
            return;
        }
...
void PSTOperationsDialog::signTransaction()
{
    ...
    if (err) {
        showStatus(tr("Failed to sign transaction: %1")
            .arg(QString::fromStdString(PSTErrorString(*err).translated)), StatusLevel::ERR);
        return;
    }
...
QString PSTOperationsDialog::renderTransaction(const PartiallySignedTransaction &pstx)
{
...
size_t PSTOperationsDialog::couldSignInputs(const PartiallySignedTransaction &pstx) {
    ...
    size_t n_signed;
    bool complete;
    const auto err{m_wallet_model->wallet().fillPST(SIGHASH_ALL, /*sign=*/false, /*bip32derivs=*/false, &n_signed, m_transaction_data, complete)};
