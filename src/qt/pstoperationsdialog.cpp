...
void PSTOperationsDialog::openWithPST(PartiallySignedTransaction pstx)
{
    ...
    bool complete;
    size_t n_could_sign;
    FinalizePST(pstx);  // Make sure all existing signatures are fully combined before checking for completeness.
    TransactionError err = m_wallet_model->wallet().fillPST(SIGHASH_ALL, false /* sign */, true /* bip32derivs */,  m_transaction_data, complete, &n_could_sign);
...
void PSTOperationsDialog::signTransaction()
{
    bool complete;
    size_t n_signed;
    TransactionError err = m_wallet_model->wallet().fillPST(SIGHASH_ALL, true /* sign */, true /* bip32derivs */, m_transaction_data, complete, &n_signed);
...
void PSTOperationsDialog::saveTransaction() {
    ...
    for (const CTxOut& out : m_transaction_data.tx->vout) {
        ...
        QString amount = FreicoinUnits::format(m_wallet_model->getOptionsModel()->getDisplayUnit(), out.GetReferenceValue());
    ...
    QString filename = GUIUtil::getSaveFileName(this,
        ...
        tr("Partially Signed Transaction (Binary) (*.pst)"), &selected_filter);
    if (filename.isEmpty()) {
        return;
    }
...
std::string PSTOperationsDialog::renderTransaction(const PartiallySignedTransaction &pstx)
{
    ...
    for (const CTxOut& out : pstx.tx->vout) {
        ...
        tx_description.append(tr(" * Sends %1 to %2")
            .arg(FreicoinUnits::formatWithUnit(FreicoinUnits::FRC, out.GetReferenceValue()))
    ...
    if (!*analysis.fee) {
        ...
        tx_description.append(FreicoinUnits::formatWithUnit(FreicoinUnits::FRC, *analysis.fee));
...
size_t PSTOperationsDialog::couldSignInputs(const PartiallySignedTransaction &pstx) {
    size_t n_signed;
    bool complete;
    TransactionError err = m_wallet_model->wallet().fillPST(SIGHASH_ALL, false /* sign */, false /* bip32derivs */, m_transaction_data, complete, &n_signed);
