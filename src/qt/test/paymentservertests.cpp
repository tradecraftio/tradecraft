...
void PaymentServerTests::paymentServerTests()
{
    ...
    std::size_t randDataSize = static_cast<std::size_t>(BIP70_MAX_PAYMENTREQUEST_SIZE) + 1;
    std::string randDataStr(randDataSize, '\0');
    unsigned char* randData = reinterpret_cast<unsigned char *>(&randDataStr[0]);
    GetRandBytes(randData, randDataSize);
    // Write data to a temp file:
    QTemporaryFile tempFile;
    tempFile.open();
    tempFile.write((const char*)randData, randDataSize);
    tempFile.close();
    // compares 50001 <= BIP70_MAX_PAYMENTREQUEST_SIZE == false
    QCOMPARE(PaymentServer::verifySize(tempFile.size()), false);

#if 0 // Test needs to be updated for new MAX_MONEY
