...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total freicoin balance of the wallet\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"freicoinaddress\"     (string, required) The freicoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"freicoinaddress\", (string) The freicoin address validated\n"
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are freicoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) freicoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
    ...
    CFreicoinAddress address(innerID);
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"freicoinaddress\"  (string, required) The freicoin address to use for the signature.\n"
...
UniValue createmerkleproof(const UniValue& params, bool fHelp)
{
    using std::swap;
    ...
    if (params.size() > 2) {
        prehashed = params[2].get_bool();
    }

    bool include_all = false;
    std::set<std::size_t> pos;
    if (params.size() > 1 && !params[1].isNull()) {
        UniValue v = params[1].get_array();
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "util",               "createmerkleproof",      &createmerkleproof,      true  },
