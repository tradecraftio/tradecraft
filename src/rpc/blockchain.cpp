...
UniValue blockToJSON(BlockManager& blockman, const CBlock& block, const CBlockIndex& tip, const CBlockIndex& blockindex, TxVerbosity verbosity)
{
    ...
    result.pushKV("strippedsize", (int)::GetSerializeSize(BLK_NO_AUXPOW_NOR_WITNESS(block)));
    result.pushKV("size", (int)::GetSerializeSize(BLK_WITH_AUXPOW_AND_WITNESS(block)));
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    {
        DataStream ssBlock;
        ssBlock << BLKHDR_WITH_AUXPOW(pblockindex->GetBlockHeader());
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (maybe_stats.has_value()) {
        ...
        if (!stats.index_used) {
            ...
            ret.pushKV("total_unspendable_value", ValueFromAmount(stats.total_unspendable_value));
            ...
            block_info.pushKV("unspendable", ValueFromAmount(stats.total_unspendable_value - prev_stats.total_unspendable_value));
    ...
    const fs::path& temppath)
{
    ...
    SnapshotMetadata metadata{tip->GetBlockHash(), final_tx, maybe_stats->coins_count};
