    ...
    for (size_t i = 0; i < block.vtx.size(); ++i) {
        ...
        if (loop_outputs) {
            for (const CTxOut& out : tx->vout) {
                ...
                size_t out_size = GetSerializeSize(out, SER_NETWORK, PROTOCOL_VERSION) + PER_UTXO_OVERHEAD;
        ...
        if (loop_inputs) {
            ...
            for (const Coin& coin: txundo.vprevout) {
                ...
                size_t prevout_size = GetSerializeSize(prevoutput, SER_NETWORK, PROTOCOL_VERSION) + PER_UTXO_OVERHEAD;
    ...
    const fs::path& temppath)
{
    ...
    SnapshotMetadata metadata{tip->GetBlockHash(), final_tx, maybe_stats->coins_count, tip->nChainTx};
