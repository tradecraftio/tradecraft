...
static bool GenerateBlock(ChainstateManager& chainman, CBlock& block, uint64_t& max_tries, unsigned int& extra_nonce, uint256& block_hash)
{
    ...
    if (!block.m_aux_pow.IsNull()) {
        {
            ...
            IncrementExtraNonceAux(block, extranonceaux);
        }
        while (max_tries > 0 && block.nNonce < std::numeric_limits<uint32_t>::max() && !CheckAuxiliaryProofOfWork(block, Params().GetConsensus())) {
    ...
    }
    {
        ...
        if (!block.m_aux_pow.IsNull()) {
            aux_hash2 = block.GetAuxiliaryHash(Params().GetConsensus()).second;
        }
    ...
    return RPCHelpMan{"getblocktemplate",
                ...
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        ...
                        {RPCResult::Type::OBJ, "finaltx", "information necessary to construct the block-final transaction",
                            {
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    { "mining",             "getnetworkhashps",       &getnetworkhashps,       {"height"} },
