...
static UniValue generateBlocks(const CTxMemPool& mempool, const CScript& coinbase_script, int nGenerate, uint64_t nMaxTries)
{
    ...
    while (nHeight < nHeightEnd && !ShutdownRequested())
    {
        ...
        while (nMaxTries > 0 && pblock->nNonce < std::numeric_limits<uint32_t>::max() && !CheckProofOfWork(pblock->GetHash(), pblock->nBits, 0, Params().GetConsensus()) && !ShutdownRequested()) {
            ++pblock->nNonce;
            --nMaxTries;
        }
...
static UniValue generatetoaddress(const JSONRPCRequest& request)
{
            ...
            + "If you are running the freicoin wallet, you can get a new address to send the newly generated freicoin to with:\n"
...
static UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (!Params().MineBlocksOnDemand() && g_rpc_node->connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
