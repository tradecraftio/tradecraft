...
static bool CheckInputsFromMempoolAndCache(const CTransaction& tx, TxValidationState& state,
                const CCoinsViewCache& view, const Consensus::Params& params, const CTxMemPool& pool,
                ...
                EXCLUSIVE_LOCKS_REQUIRED(cs_main, pool.cs)
{
    ...
    return CheckInputScripts(tx, state, view, params, /* per_input_adjustment = */ 0, flags, /* cacheSigStore= */ true, /* cacheFullScriptStore= */ true, txdata);
}
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!CheckTransaction(tx, state, Consensus::NONE)) {
...
bool MemPoolAccept::PolicyScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    if (!CheckInputScripts(tx, state, m_view, args.m_chainparams.GetConsensus(), /* per_input_adjustment = */ 0, scriptVerifyFlags, true, false, ws.m_precomputed_txdata)) {
        // SCRIPT_VERIFY_CLEANSTACK requires SCRIPT_VERIFY_WITNESS, so we
        // need to turn both off, and compare against just turning off CLEANSTACK
        // to see if the failure is specifically due to witness validation.
        TxValidationState state_dummy; // Want reported failures to be from first CheckInputScripts
        if (!tx.HasWitness() && CheckInputScripts(tx, state_dummy, m_view, args.m_chainparams.GetConsensus(), /* per_input_adjustment = */ 0, scriptVerifyFlags & ~(SCRIPT_VERIFY_WITNESS | SCRIPT_VERIFY_CLEANSTACK), true, false, ws.m_precomputed_txdata) &&
                !CheckInputScripts(tx, state_dummy, m_view, args.m_chainparams.GetConsensus(), /* per_input_adjustment = */ 0, scriptVerifyFlags & ~SCRIPT_VERIFY_CLEANSTACK, true, false, ws.m_precomputed_txdata)) {
...
bool MemPoolAccept::ConsensusScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    if (!CheckInputsFromMempoolAndCache(tx, state, m_view, chainparams.GetConsensus(), m_pool, currentBlockScriptVerifyFlags,
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // Check for activation of rule changes
    const Consensus::RuleSet rules = pindex->pprev ? GetActiveRules(m_params.GetConsensus(), *pindex->pprev) : Consensus::NONE;
    ...
    bool use_alu = false;
    if (pindex->nHeight >= m_params.GetConsensus().alu_activation_height) {
        use_alu = true;
    }
    ...
    bool enforce_block_final = pindex->pprev && DeploymentActiveAfter(pindex->pprev, m_params.GetConsensus(), Consensus::DEPLOYMENT_FINALTX);
    ...
    const bool initial_block_final = enforce_block_final && (!pindex->pprev->pprev || !DeploymentActiveAfter(pindex->pprev->pprev, m_params.GetConsensus(), Consensus::DEPLOYMENT_FINALTX));
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (!Consensus::CheckTxInputs(tx, tx_state, view, m_params.GetConsensus(), !truncate_inputs + !use_alu, pindex->nHeight, rules, txfee)) {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            TxValidationState tx_state;
            if (fScriptChecks && !CheckInputScripts(tx, tx_state, view, m_params.GetConsensus(), !truncate_inputs + !use_alu, flags, fCacheResults, fCacheResults, txsdata[i], g_parallel_script_checks ? &vChecks : nullptr)) {
                // Any transaction validation failure in ConnectBlock is a block consensus failure
                state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
                              tx_state.GetRejectReason(), tx_state.GetDebugMessage());
...
bool CChainState::ActivateBestChainStep(BlockValidationState& state, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    ...
    if (m_mempool) m_mempool->check(this->CoinsTip(), this->m_chain.Height() + 1, m_params.GetConsensus());
...
static bool CheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    if (!block.m_aux_pow.IsNull()) {
        ...
        if (aux_pow.m_midstate_buffer.size() >= 64) {
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "auxpow-midstate-buffer", "auxiliary proof-of-work midstate buffer is too large");
        }

        if (aux_pow.m_midstate_buffer.size() != aux_pow.m_midstate_length % 64) {
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "auxpow-midstate-length", "auxiliary proof-of-work midstate buffer doesn't match anticipated length");
        }

        if (aux_pow.m_aux_branch.size() > MAX_AUX_POW_BRANCH_LENGTH) {
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "auxpow-merkle-branch", "auxiliary proof-of-work Merkle branch is too long");
        }

        if (aux_pow.m_commit_branch.size() > MAX_AUX_POW_COMMIT_BRANCH_LENGTH) {
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "auxpow-commit-branch", "auxiliary proof-of-work Merkle map path is too long");
        }
        ...
        if (nbits >= 256) {
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "auxpow-commit-branch-bits", "auxiliary proof-of-work Merkle map path is greater than 256 bits");
        }
    }
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (fCheckMerkleRoot) {
        ...
        if (!block.m_aux_pow.IsNull()) {
            ...
            if (mutated) {
                return state.Invalid(BlockValidationResult::BLOCK_MUTATED, "bad-auxpow-mutated", "auxiliary proof-of-work header is non-canonical (mutated)");
            }
            ...
            if (block.vtx.empty() || block.vtx[0]->vin.empty() || (block.vtx[0]->vin[0].scriptSig.size() < 32) || memcmp(aux_hash.second.begin(), &(block.vtx[0]->vin[0].scriptSig.end()-32)[0], 32)) {
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-auxpow-commit", "incorrect commitment to auxiliary proof-of-work in coinbase string");
            }
            ...
            if (ComputeMerkleRootFromBranch(cb.GetHash(), cb_branch, 0) != block.m_aux_pow.m_commit_hash_merkle_root) {
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-commit-txnmrklroot", "block template hashMerkleRoot mismatch");
            }
        }
    }
    ...
    for (const auto& tx : block.vtx) {
        TxValidationState tx_state;
        if (!CheckTransaction(*tx, tx_state, rules)) {
...
void UpdateUncommittedBlockStructures(CBlock& block, const CBlockIndex* pindexPrev, const Consensus::Params& consensusParams)
{
    ...
    if (GetWitnessCommitment(block, nullptr, nullptr) && DeploymentActiveAfter(pindexPrev, consensusParams, Consensus::DEPLOYMENT_SEGWIT) && !block.vtx[0]->HasWitness()) {
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    if (!(rules & Consensus::PROTOCOL_CLEANUP) && (
        (block.nVersion < 2 && DeploymentActiveAfter(pindexPrev, consensusParams, Consensus::DEPLOYMENT_HEIGHTINCB)) ||
    ...
    if (!DeploymentActiveAfter(pindexPrev, consensusParams, Consensus::DEPLOYMENT_AUXPOW) && !block.m_aux_pow.IsNull()) {
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    int nLockTimeFlags = 0;
    if (DeploymentActiveAfter(pindexPrev, consensusParams, Consensus::DEPLOYMENT_LOCKTIME)) {
        assert(pindexPrev != nullptr);
    ...
    if (!(rules & Consensus::PROTOCOL_CLEANUP)) {
        if (!block.vtx.empty() && !IsFinalTx(*block.vtx[0], nHeight, nLockTimeCutoff)) {
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-txns-nonfinal", "non-final coinbase");
        }
    }
    ...
    if (!(rules & Consensus::PROTOCOL_CLEANUP) && DeploymentActiveAfter(pindexPrev, consensusParams, Consensus::DEPLOYMENT_HEIGHTINCB))
    {
    ...
    if (DeploymentActiveAfter(pindexPrev, consensusParams, Consensus::DEPLOYMENT_SEGWIT)) {
        ...
        if (GetWitnessCommitment(block, &witnesspath, &commithash)) {
            ...
            {
                return state.Invalid(BlockValidationResult::BLOCK_MUTATED, "bad-witness-branch-size", strprintf("%s : invalid witness branch size", __func__));
            }
...
bool ChainstateManager::ProcessNewBlock(const CChainParams& chainparams, const std::shared_ptr<const CBlock>& block, bool force_processing, bool* new_block)
{
...

    {
        ...
        // https://lists.linuxfoundation.org/pipermail/freicoin-dev/2019-February/016697.html.  Because CheckBlock() is
...
MempoolAcceptResult ChainstateManager::ProcessTransaction(const Consensus::Params& consensusparams, const CTransactionRef& tx, bool test_accept)
{
    AssertLockHeld(cs_main);
    ...
    active_chainstate.GetMempool()->check(active_chainstate.CoinsTip(), active_chainstate.m_chain.Height() + 1, consensusparams);
    return result;
}
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs)
{
    ...
    const bool enforce_block_final = pindex->pprev && DeploymentActiveAfter(pindex->pprev, m_params.GetConsensus(), Consensus::DEPLOYMENT_FINALTX);
    const bool initial_block_final = enforce_block_final && pindex->pprev->pprev && !DeploymentActiveAfter(pindex->pprev->pprev, m_params.GetConsensus(), Consensus::DEPLOYMENT_FINALTX);
    BlockFinalTxEntry entry;
    if (initial_block_final) {
        entry.hash = block.vtx[0]->GetHash();
...
bool CChainState::NeedsRedownload() const
{
    ...
    if (block != nullptr && block->pprev != nullptr && DeploymentActiveAt(*block, m_params.GetConsensus(), Consensus::DEPLOYMENT_FINALTX)) {
