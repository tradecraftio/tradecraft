...
static bool CheckInputsFromMempoolAndCache(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& view, const Consensus::Params& params, const CTxMemPool& pool,
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!CheckTransaction(tx, state, true, false))
    ...
    CAmount nFees = 0;
    if (!Consensus::CheckTxInputs(tx, state, m_view, chainparams.GetConsensus(), 0, GetSpendHeight(m_view), 0, nFees)) {
    ...
    if (!m_pool.CalculateMemPoolAncestors(*entry, setAncestors, m_limit_ancestors, m_limit_ancestor_size, m_limit_descendants, m_limit_descendant_size, errString)) {
        ...
        // this, see https://lists.linuxfoundation.org/pipermail/freicoin-dev/2018-November/016518.html
...
bool MemPoolAccept::ConsensusScriptChecks(ATMPArgs& args, Workspace& ws, PrecomputedTransactionData& txdata)
{
    ...
    if (!CheckInputsFromMempoolAndCache(tx, state, m_view, chainparams.GetConsensus(), m_pool, currentBlockScriptVerifyFlags, true, txdata)) {
...
bool MemPoolAccept::Finalize(ATMPArgs& args, Workspace& ws)
{
    ...
    // - it isn't a replacement transaction (may not be widely supported)
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (nHeight < consensusParams.equilibrium_height) {
    ...
    nSubsidy += consensusParams.perpetual_subsidy;

    // For compatibility with unit and regression tests inherited from
    // upstream, bitcoin's regtest reward schedule is preserved:
    // initially 50 FRC per block, halved every 150 blocks.
    if (consensusParams.nSubsidyHalvingInterval) {
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    ...
    if (pindex->pprev && IsProtocolCleanupActive(consensusparams, *pindex->pprev)) {
        flags |= SCRIPT_VERIFY_PROTOCOL_CLEANUP;
    }
    if (pindex->pprev && IsSizeExpansionActive(consensusparams, *pindex->pprev)) {
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Check for activation of rule changes
    const Consensus::RuleSet rules = pindex->pprev ? GetActiveRules(chainparams.GetConsensus(), *pindex->pprev) : 0;
    ...
    if (initial_block_final) {
        ...
        if (block.vtx[0]->vout.empty()) {
            return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "bad-cb-missing-outputs", "block-final activation coinbase has no outputs");
        }
        ...
        if (!IsTriviallySpendable(*block.vtx[0], 0, flags|SCRIPT_VERIFY_WITNESS|SCRIPT_VERIFY_CLEANSTACK)) {
            return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "bad-cb-missing-initial-block-final-output", "block-final activation coinbase missing trivial output");
        }
    ...
    if (enforce_block_final && !any_prev_final_utxos) {
        ...
        return AbortNode(state, strprintf("%s: prior block-final tx hash %s not found; corruption likely!", __func__, entry.hash.GetHex()), _("Database corruption likely.  Try restarting with `-reindex=1`.").translated);
    }
    ...
    if (enforce_block_final) {
        ...
        if (block.vtx.size() < 2) {
            return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "missing-block-final-tx", "missing block-final transaction");
        }
        ...
        for (const CTxIn& txin : final_tx.vin) {
            ...
            return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "block-final-spend-invalid", "block-final transaction makes invalid spend");
        }
        ...
        if (coin.IsSpent()) {
            // Should never happen.
            return AbortNode(state, strprintf("%s: unspent output of prior block-final tx outpoint %s:0 not found; corruption likely!", __func__, entry.hash.GetHex()), _("Database corruption likely.  Try restarting with `-reindex=1`.").translated);
        }
        ...
        if (!coin.IsCoinBase() && (spends_from_prior_tx < entry.size)) {
            return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "block-final-missing-prior-input", "missing txin of prior block-final transaction");
        }
        ...
        if (final_tx.vout.size() > final_tx.vin.size()) {
            return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "block-final-excess-output", "too many outputs for block-final transaction");
        }
        ...
        for (const CTxOut& txout : final_tx.vout) {
            if (!IsTriviallySpendable(final_tx, 0, flags|SCRIPT_VERIFY_WITNESS|SCRIPT_VERIFY_CLEANSTACK)) {
                return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "block-final-nontrivial-ouput", "block-final txout not trivially spendable");
            }
        }
    }
    ...
    if (block.vtx[0]->lock_height != pindex->nHeight)
        return state.Invalid(ValidationInvalidReason::CONSENSUS, error("ConnectBlock() : coinbase lock_height (%d) != block height (%d)", block.vtx[0]->lock_height, pindex->nHeight), REJECT_INVALID, "bad-cb-lock-height");
    ...
    EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    if (!::ChainstateActive().IsInitialBlockDownload())
    {
        ...
        for (int i = 0; i < 100 && pindex != nullptr; i++)
        {
            ...
            if (pindex->nVersion > VERSIONBITS_LAST_OLD_BLOCK_VERSION && ((pindex->nVersion & ~nExpectedVersion) & 0xe0001fff) != 0)
                ++nUpgraded;
            pindex = pindex->pprev;
        }
        if (nUpgraded > 0)
...
bool CChainState::ActivateBestChainStep(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    ...
    mempool.check(&CoinsTip(), chainparams.GetConsensus());
...
static bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    if (!block.m_aux_pow.IsNull()) {
        ...
        if (aux_pow.m_midstate_buffer.size() >= 64) {
            return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, "auxpow-midstate-buffer", "auxiliary proof-of-work midstate buffer is too large");
        }

        if (aux_pow.m_midstate_buffer.size() != aux_pow.m_midstate_length % 64) {
            return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, "auxpow-midstate-length", "auxiliary proof-of-work midstate buffer doesn't match anticipated length");
        }

        if (aux_pow.m_aux_branch.size() > MAX_AUX_POW_BRANCH_LENGTH) {
            return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, "auxpow-merkle-branch", "auxiliary proof-of-work Merkle branch is too long");
        }

        if (aux_pow.m_commit_branch.size() > MAX_AUX_POW_COMMIT_BRANCH_LENGTH) {
            return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, "auxpow-commit-branch", "auxiliary proof-of-work Merkle map path is too long");
        }
        ...
        if (nbits >= 256) {
            return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, "auxpow-commit-branch-bits", "auxiliary proof-of-work Merkle map path is greater than 256 bits");
        }
    }

    // Check proof of work matches claimed amount
    if (fCheckPOW && !CheckAuxiliaryProofOfWork(block, consensusParams)) {
        return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, "aux-pow-invalid", "auxiliary proof of work failed");
    }
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (fCheckMerkleRoot) {
        ...
        if (!block.m_aux_pow.IsNull()) {
            ...
            if (mutated) {
                return state.Invalid(ValidationInvalidReason::BLOCK_MUTATED, false, REJECT_INVALID, "bad-auxpow-mutated", "auxiliary proof-of-work header is non-canonical (mutated)");
            }
            ...
            if (block.vtx.empty() || block.vtx[0]->vin.empty() || (block.vtx[0]->vin[0].scriptSig.size() < 32) || memcmp(aux_hash.second.begin(), &(block.vtx[0]->vin[0].scriptSig.end()-32)[0], 32)) {
                return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "bad-auxpow-commit", "incorrect commitment to auxiliary proof-of-work in coinbase string");
            }
            ...
            if (ComputeMerkleRootFromBranch(cb.GetHash(), cb_branch, 0) != block.m_aux_pow.m_commit_hash_merkle_root) {
                return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "bad-commit-txnmrklroot", "block template hashMerkleRoot mismatch");
            }
        }
    }
    ...
    if (block.vtx.empty() || block.vtx.size() * WITNESS_SCALE_FACTOR > max_block_weight || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS | SERIALIZE_BLOCK_NO_AUX_POW) * WITNESS_SCALE_FACTOR > max_block_weight)
    ...
    if (!(rules & Consensus::PROTOCOL_CLEANUP) && (nSigOps * WITNESS_SCALE_FACTOR > MAX_BLOCK_SIGOPS_COST))
...
void UpdateUncommittedBlockStructures(CBlock& block, const CBlockIndex* pindexPrev, const Consensus::Params& consensusParams)
{
    ...
    if (GetWitnessCommitment(block, nullptr, nullptr) && IsWitnessEnabled(pindexPrev, consensusParams) && !block.vtx[0]->HasWitness()) {
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    // Check for activation of rule changes
    const Consensus::RuleSet rules = pindexPrev ? GetActiveRules(consensusParams, *pindexPrev) : 0;
    ...
    if (!IsMergeMiningEnabled(pindexPrev, consensusParams) && !block.m_aux_pow.IsNull()) {
        return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_OBSOLETE, "merge-mined-pre", "rejected merge-mined block before activation");
    }
    ...
    if (IsMergeMiningEnabled(pindexPrev, consensusParams) && block.m_aux_pow.IsNull()) {
        ...
        // ignore and move on...
        return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_OBSOLETE, "native-mined-post", "rejected non-merge-mined block after activation");
    }
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    int nLockTimeFlags = 0;
    if (nHeight >= consensusParams.LockTimeHeight) {
        assert(pindexPrev != nullptr);
    ...
    if (!(rules & Consensus::PROTOCOL_CLEANUP)) {
        if (!block.vtx.empty() && !IsFinalTx(*block.vtx[0], nHeight, nLockTimeCutoff)) {
            return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "bad-txns-nonfinal", "non-final coinbase");
        }
    }
    ...
    if (!(rules & Consensus::PROTOCOL_CLEANUP) && (nHeight >= consensusParams.BIP34Height))
    {
    ...
    if (nHeight >= consensusParams.SegwitHeight) {
        ...
        if (GetWitnessCommitment(block, &witnesspath, &commithash)) {
            ...
            if (witnesspath == 0) {
                return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "bad-witness-path", strprintf("%s : witness commitment path is not present", __func__));
            }
            ...
            if (   block.vtx[0]->vin.empty()
                ...
                || block.vtx[0]->vin[0].scriptWitness.stack[0].size() != 32*witnessdepth)
            {
                return state.Invalid(ValidationInvalidReason::BLOCK_MUTATED, false, REJECT_INVALID, "bad-witness-branch-size", strprintf("%s : invalid witness branch size", __func__));
            }
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs, const CChainParams& params)
{
    ...
    const bool enforce_block_final = pindex->pprev && IsFinalTxEnforced(pindex->pprev, params.GetConsensus());
    const bool initial_block_final = enforce_block_final && pindex->pprev->pprev && !IsFinalTxEnforced(pindex->pprev->pprev, params.GetConsensus());
    BlockFinalTxEntry entry;
    if (initial_block_final) {
        entry.hash = block.vtx[0]->GetHash();
        entry.size = 1;
