                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        if (!CheckInputsFromMempoolAndCache(tx, state, view, chainparams.GetConsensus(), pool, currentBlockScriptVerifyFlags, true, txdata)) {
        ...
        // - it isn't a replacement transaction
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (VersionBitsState(pindex->pprev, chainparams.GetConsensus(), Consensus::DEPLOYMENT_LOCKTIME, versionbitscache) == ThresholdState::ACTIVE) {
    ...
    if (initial_block_final) {
        ...
        if (!IsTriviallySpendable(*block.vtx[0], 0, flags|SCRIPT_VERIFY_WITNESS|SCRIPT_VERIFY_CLEANSTACK)) {
            return state.DoS(100, error("%s: coinbase missing initial output for block-final rules", __func__),
                             REJECT_INVALID, "bad-cb-missing-initial-block-final-output");
        }
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    int nLockTimeFlags = 0;
    if (VersionBitsState(pindexPrev, consensusParams, Consensus::DEPLOYMENT_LOCKTIME, versionbitscache) == ThresholdState::ACTIVE) {
        assert(pindexPrev != nullptr);
