...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs, RuleSet rules)
{
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (!CheckInputs(tx, state, view, Params().GetConsensus(), 0, true, scriptVerifyFlags, true, txdata)) {
            // SCRIPT_VERIFY_CLEANSTACK requires SCRIPT_VERIFY_WITNESS, so we
            // need to turn both off, and compare against just turning off CLEANSTACK
            // to see if the failure is specifically due to witness validation.
            CValidationState stateDummy; // Want reported failures to be from first CheckInputs
            if (!tx.HasWitness() && CheckInputs(tx, stateDummy, view, Params().GetConsensus(), 0, true, scriptVerifyFlags & ~(SCRIPT_VERIFY_WITNESS | SCRIPT_VERIFY_CLEANSTACK), true, txdata) &&
                !CheckInputs(tx, stateDummy, view, Params().GetConsensus(), 0, true, scriptVerifyFlags & ~SCRIPT_VERIFY_CLEANSTACK, true, txdata)) {
        ...
        if (!CheckInputs(tx, state, view, Params().GetConsensus(), 0, true, MANDATORY_SCRIPT_VERIFY_FLAGS, true, txdata))
        {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s FRC additional fees, %d delta bytes\n",
        ...
        // replacement transaction, the node is not behind, and the transaction
        // is not dependent on any other transactions in the mempool.
...
namespace Consensus {
bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, const Consensus::Params& params, int per_input_adjustment, int nSpendHeight, RuleSet rules)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if ((coins->vout[prevout.n].GetReferenceValue() || !(rules & PROTOCOL_CLEANUP)) && !params.bitcoin_mode && (tx.lock_height < coins->refheight)) {
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-non-monotonic-lock-height", false, strprintf("%s: tx.lock_height %d < coin.refheight %d", __func__, tx.lock_height, coins->refheight));
            }

            // Check for negative or overflow input values
            nInput = coins->GetPresentValueOfOutput(prevout.n, tx.lock_height) + per_input_adjustment;
            nValueIn += nInput;
...
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, const Consensus::Params& params, int per_input_adjustment, bool fScriptChecks, unsigned int flags, bool cacheStore, PrecomputedTransactionData& txdata, std::vector<CScriptCheck> *pvChecks)
{
    ...
    if (!tx.IsCoinBase())
    {
        if (!Consensus::CheckTxInputs(tx, state, inputs, params, per_input_adjustment, GetSpendHeight(inputs), rules))
            return false;
...
bool IsTriviallySpendable(const CCoins& from, const COutPoint& prevout, unsigned int flags)
{
    ...
    CScriptCheck check(from, to, 0, flags, false, &txdata);
    return check();
}
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (pindex->nHeight >= chainparams.GetConsensus().verify_coinbase_lock_time_activation_height) {
        if (!block.vtx.empty() && (block.vtx[0]->nLockTime != pindex->pprev->GetMedianTimePast())) {
            return state.DoS(100, false, REJECT_INVALID, "coinbase-locktime-not-mtp");
        }
    }
    ...
    if (enforce_block_final) {
        // The view contains the hash of the last block-final transaction, which
        // necessarily *MUST* exist in the unspent txout database, since it
        // occured in a prior block.
        const CCoins* prev_final_tx = view.AccessCoins(view.GetFinalTx());
        if (!prev_final_tx) {
            // This state of affairs shouldn't happen, because the function
            // RewindBlockIndex is called by the initialization code, and
            // rewinds to the point of activation if this condition is
            // encountered, which one might expect to occur when performing a
            // system upgrade from a prior node version to one which supports
            // validation of block-final transaction rules, but *after* rule
            // activation.  Should RewindBlockIndex somehow fail to be called,
            // normal tip validation in init.cpp will halt on this error and
            // notify that the user that their block database is corrupted,
        ...
        if (prev_final_tx->IsCoinBase() && (pindex->nHeight - prev_final_tx->nHeight < COINBASE_MATURITY)) {
            // Still maturing. Nothing to do.
            enforce_block_final = false;
        } else {
            // Make sure the block contains *at least* 2 transactions, the
            // coinbase and the block-final transaction. This is not stricly
            ...
            for (const CTxIn& txin : final_tx.vin) {
                ...
                // Fetching the UTXO record will fail for any outputs sourced
                // from the current block, and all such spends are allowed (for
                // future expansion purposes):
                const CCoins* from = view.AccessCoins(txin.prevout.hash);
                if (!from) {
                    continue;
                }
    ...
    if (block.vtx[0]->lock_height != pindex->nHeight)
        return state.DoS(100, error("ConnectBlock() : coinbase lock_height (%d) != block height (%d)",
                                    block.vtx[0]->lock_height, pindex->nHeight),
                                    REJECT_INVALID, "bad-cb-lock-height");
    ...
    if (initial_block_final) {
        view.SetFinalTx(block.vtx.front()->GetHash());
    }
    if (enforce_block_final) {
        view.SetFinalTx(block.vtx.back()->GetHash());
    }
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int bit = 0; bit < VERSIONBITS_NUM_BITS; bit++) {
            ...
            ThresholdState state = checker.GetStateFor(pindex, chainParams.GetConsensus(), warningcache[bit], false);
...
static bool ActivateBestChainStep(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    ...
    mempool.check(pcoinsTip, chainparams.GetConsensus());
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, int64_t nAdjustedTime, bool fCheckPOW)
{
    ...
    // Check proof of work
    if (!(rules & PROTOCOL_CLEANUP)) {
        if (block.nBits != GetNextWorkRequired(pindexPrev, &block, consensusParams, rules)) {
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    int nLockTimeFlags = 0;
    if (VersionBitsState(pindexPrev, consensusParams, Consensus::DEPLOYMENT_LOCKTIME, versionbitscache) == THRESHOLD_ACTIVE) {
    ...
    if (IsWitnessEnabled(pindexPrev, consensusParams)) {
        ...
        if (GetWitnessCommitment(block, &witnesspath, &commithash)) {
            ...
            if (witnesspath == 0) {
                return state.DoS(100, false, REJECT_INVALID, "bad-witness-path", true, strprintf("%s : witness commitment path is not present", __func__));
            }
            ...
            {
                return state.DoS(100, false, REJECT_INVALID, "bad-witness-branch-size", true, strprintf("%s : invalid witness branch size", __func__));
            }
