...
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, const Consensus::Params& params, int per_input_adjustment, bool fScriptChecks, unsigned int flags, bool cacheSigStore, bool cacheFullScriptStore, PrecomputedTransactionData& txdata, std::vector<CScriptCheck> *pvChecks = nullptr);
...
static bool CheckInputsFromMempoolAndCache(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &view, const Consensus::Params& params, CTxMemPool& pool,
    ...
    return CheckInputs(tx, state, view, params, 0, true, flags, cacheSigStore, true, txdata);
}
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if (!CheckInputs(tx, state, view, Params().GetConsensus(), 0, true, scriptVerifyFlags, true, false, txdata)) {
            // SCRIPT_VERIFY_CLEANSTACK requires SCRIPT_VERIFY_WITNESS, so we
            // need to turn both off, and compare against just turning off CLEANSTACK
            // to see if the failure is specifically due to witness validation.
            CValidationState stateDummy; // Want reported failures to be from first CheckInputs
            if (!tx.HasWitness() && CheckInputs(tx, stateDummy, view, Params().GetConsensus(), 0, true, scriptVerifyFlags & ~(SCRIPT_VERIFY_WITNESS | SCRIPT_VERIFY_CLEANSTACK), true, false, txdata) &&
                !CheckInputs(tx, stateDummy, view, Params().GetConsensus(), 0, true, scriptVerifyFlags & ~SCRIPT_VERIFY_CLEANSTACK, true, false, txdata)) {
        ...
        if (!CheckInputsFromMempoolAndCache(tx, state, view, Params().GetConsensus(), pool, currentBlockScriptVerifyFlags, true, txdata))
        {
            ...
            if (!(~scriptVerifyFlags & currentBlockScriptVerifyFlags)) {
                ...
                if (!CheckInputs(tx, state, view, Params().GetConsensus(), 0, true, MANDATORY_SCRIPT_VERIFY_FLAGS, true, false, txdata)) {
...
bool CScriptCheck::operator()() {
    ...
    return VerifyScript(scriptSig, scriptPubKey, witness, nFlags, CachingTransactionSignatureChecker(ptxTo, nIn, amount, refheight, cacheStore, *txdata), &error);
}
    ...
    if (!tx.IsCoinBase())
    {
        ...
        if (fScriptChecks) {
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                // Verify signature
                CScriptCheck check(scriptPubKey, amount, refheight, tx, i, flags, cacheSigStore, &txdata);
                if (pvChecks) {
                    ...
                    if (flags & STANDARD_NOT_MANDATORY_VERIFY_FLAGS) {
                        ...
                        // non-upgraded nodes.
                        CScriptCheck check2(scriptPubKey, amount, refheight, tx, i,
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    if (enforce_block_final && !any_prev_final_utxos) {
        ...
        // node version to one which supports validation of block-final
        // transaction rules, but *after* rule activation.  Should
        // RewindBlockIndex somehow fail to be called, normal tip validation in
        // init.cpp will halt on this error and notify that the user that their
        // block database is corrupted, which is fixed by starting with
        // -reindex=1.
        state.SetCorruptionPossible();
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (!CheckInputs(tx, state, view, chainparams.GetConsensus(), !truncate_inputs + !use_alu, fScriptChecks, flags, fCacheResults, fCacheResults, txdata[i], nScriptCheckThreads ? &vChecks : nullptr))
...
bool RewindBlockIndex(const CChainParams& params)
{
    ...
    while (nHeight <= chainActive.Height()) {
        if (IsFinalTxEnforced(chainActive[nHeight - 1], params.GetConsensus()) && pcoinsTip->GetFinalTx().hash.IsNull()) {
            break;
        }
