...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!Consensus::CheckTxInputs(tx, state, m_view, chainparams.GetConsensus(), /* per_input_adjustment = */ 0, m_active_chainstate.m_blockman.GetSpendHeight(m_view), Consensus::NONE, ws.m_base_fees)) {
...
bool MemPoolAccept::ConsensusScriptChecks(const ATMPArgs& args, Workspace& ws, PrecomputedTransactionData& txdata)
{
    ...
    if (!CheckInputsFromMempoolAndCache(tx, state, m_view, chainparams.GetConsensus(), m_pool, currentBlockScriptVerifyFlags, txdata, m_active_chainstate.CoinsTip())) {
...
bool CChainState::ActivateBestChainStep(BlockValidationState& state, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    ...
    if (m_mempool) m_mempool->check(*this, m_params.GetConsensus());
