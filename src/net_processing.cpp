...
static bool TipMayBeStale(const Consensus::Params &consensusParams) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    return g_last_tip_update < GetTime() - consensusParams.aux_pow_target_spacing * 3 && mapBlocksInFlight.empty();
}

static bool CanDirectFetch(const Consensus::Params &consensusParams) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    const auto& tip = ::ChainActive().Tip();
    return tip->GetBlockTime() > GetAdjustedTime() - (tip->m_aux_pow.IsNull() ? consensusParams.nPowTargetSpacing : consensusParams.aux_pow_target_spacing) * 20;
}
...
void static ProcessOrphanTx(CConnman* connman, CTxMemPool& mempool, std::set<uint256>& orphan_work_set, std::list<CTransactionRef>& removed_txn, const CChainParams& chainparams) EXCLUSIVE_LOCKS_REQUIRED(cs_main, g_cs_orphans)
{
    AssertLockHeld(cs_main);
    ...
    while (!done && !orphan_work_set.empty()) {
        ...
        mempool.check(&::ChainstateActive().CoinsTip(), chainparams.GetConsensus());
    }
}
...
bool ProcessMessage(CNode* pfrom, const std::string& msg_type, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CTxMemPool& mempool, CConnman* connman, BanMan* banman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::GETBLOCKS) {
        ...
        for (; pindex; pindex = ::ChainActive().Next(pindex))
        {
            ...
            const auto& tip = ::ChainActive().Tip();
            const int nPrunedBlocksLikelyToHave = MIN_BLOCKS_TO_KEEP - 3600 / (tip->m_aux_pow.IsNull() ? chainparams.GetConsensus().nPowTargetSpacing : chainparams.GetConsensus().aux_pow_target_spacing);
            if (fPruneMode && (!(pindex->nStatus & BLOCK_HAVE_DATA) || pindex->nHeight <= tip->nHeight - nPrunedBlocksLikelyToHave))
            {
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        if (!AlreadyHave(inv, mempool) &&
            ...
            ProcessOrphanTx(connman, mempool, pfrom->orphan_work_set, lRemovedTxn, chainparams);
        }
    ...
    if (msg_type == NetMsgType::BLOCK)
    {
        ...
        const CBlockIndex* pindex = nullptr;
        {
            ...
            pindex = m_chainman.m_blockman.LookupBlockIndex(pblock->GetHash());
        }
...
bool PeerLogicValidation::ProcessMessages(CNode* pfrom, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (!pfrom->orphan_work_set.empty()) {
        ...
        LOCK2(cs_main, g_cs_orphans);
        ProcessOrphanTx(connman, m_mempool, pfrom->orphan_work_set, removed_txn, chainparams);
    ...
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted == 0 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 24 * 60 * 60) {
                ...
                state.nHeadersSyncTimeout = GetTimeMicros() + HEADERS_DOWNLOAD_TIMEOUT_BASE + HEADERS_DOWNLOAD_TIMEOUT_PER_HEADER * (GetAdjustedTime() - pindexBestHeader->GetBlockTime())/(consensusParams.aux_pow_target_spacing);
                nSyncStarted++;
