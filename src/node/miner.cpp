...
// Copyright (c) 2009-2022 The Freicoin developers
...
int64_t UpdateTime(CBlockHeader* pblock, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (consensusParams.fPowAllowMinDifficultyBlocks) {
        pblock->nBits = GetNextWorkRequired(pindexPrev, pblock, consensusParams, pindexPrev ? GetActiveRules(consensusParams, *pindexPrev) : Consensus::NONE);
    }
...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    pblock->nBits          = GetNextWorkRequired(pindexPrev, pblock, chainparams.GetConsensus(), pindexPrev ? GetActiveRules(chainparams.GetConsensus(), *pindexPrev) : Consensus::NONE);
    pblock->nNonce         = 0;
