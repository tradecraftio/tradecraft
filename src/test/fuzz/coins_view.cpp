    ...
    LIMITED_WHILE(good_data && fuzzed_data_provider.ConsumeBool(), 10'000)
    {
        CallOneOf(
            ...
            [&] {
                ...
                bool expected_code_path = false;
                try {
                    coins_view_cache.BatchWrite(coins_map, fuzzed_data_provider.ConsumeBool() ? ConsumeUInt256(fuzzed_data_provider) : coins_view_cache.GetBestBlock(), fuzzed_data_provider.ConsumeBool() ? BlockFinalTxEntry(Txid::FromUint256(ConsumeUInt256(fuzzed_data_provider)), 1) : coins_view_cache.GetFinalTx());
                    expected_code_path = true;
                } catch (const std::logic_error& e) {
    ...
    if (fuzzed_data_provider.ConsumeBool()) {
        CallOneOf(
            ...
            [&] {
                ...
                if (Consensus::CheckTxInputs(transaction, state, coins_view_cache, Params().GetConsensus(), fuzzed_data_provider.ConsumeIntegralInRange<int>(0, std::numeric_limits<int>::max()), tx_fee_out)) {
