...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    {
        CTransaction _tx(tx);
        BOOST_CHECK(res = AcceptToMemoryPool(*m_node.mempool, tx_state, MakeTransactionRef(std::move(_tx)), nullptr, true, 0));
        BOOST_CHECK_MESSAGE(res, tx_state.GetRejectReason());
    }

    tx_state = TxValidationState();
    {
        CTransaction _tx2(tx2);
        BOOST_CHECK(!AcceptToMemoryPool(*m_node.mempool, tx_state, MakeTransactionRef(std::move(_tx2)), nullptr, true, 0));
        BOOST_CHECK_MESSAGE(res, tx_state.GetRejectReason());
    }
    ...
    BOOST_CHECK(res = TestBlockValidity(block_state, chainparams, pblocktemplate->block, ::ChainActive().Tip(), false, false));
    ...
    BOOST_CHECK(!TestBlockValidity(block_state, chainparams, pblocktemplate->block, ::ChainActive().Tip(), false, false));
    ...
    {
        CTransaction _tx2(tx2);
        BOOST_CHECK(res = AcceptToMemoryPool(*m_node.mempool, tx_state, MakeTransactionRef(std::move(_tx2)), nullptr, true, 0));
        BOOST_CHECK_MESSAGE(res, tx_state.GetRejectReason());
    }
    ...
    block_state = BlockValidationState();
    BOOST_CHECK_MESSAGE(TestBlockValidity(block_state, chainparams, pblocktemplate->block, ::ChainActive().Tip(), false, false), block_state.GetRejectReason());
