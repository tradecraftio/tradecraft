...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    BOOST_CHECK(CheckFinalTx(m_node.chainman->ActiveChain().Tip(), CTransaction(tx), STANDARD_SCRIPT_VERIFY_FLAGS));
    BOOST_CHECK(CheckFinalTx(m_node.chainman->ActiveChain().Tip(), CTransaction(tx2), STANDARD_SCRIPT_VERIFY_FLAGS));

    // But only the first transaction makes it into the mempool
    {
        CTransaction _tx(tx);
        const auto res = AcceptToMemoryPool(m_node.chainman->ActiveChainstate(), *m_node.mempool, MakeTransactionRef(std::move(_tx)), false);
...

    {
        ...
        const auto res = AcceptToMemoryPool(m_node.chainman->ActiveChainstate(), *m_node.mempool, MakeTransactionRef(std::move(_tx2)), false);
    ...
    // The block with one transaction would be valid, if mined
    {
        ...
        BOOST_CHECK(res = TestBlockValidity(block_state, chainparams, m_node.chainman->ActiveChainstate(), pblocktemplate->block, m_node.chainman->ActiveChain().Tip(), false, false));
    ...
    pblocktemplate->block.vtx.resize(3);
    {
        ...
        BlockValidationState block_state;
        BOOST_CHECK(!TestBlockValidity(block_state, chainparams, m_node.chainman->ActiveChainstate(), pblocktemplate->block, m_node.chainman->ActiveChain().Tip(), false, false));
