...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    {
        CTransaction _tx(tx);
        BOOST_CHECK(res = AcceptToMemoryPool(mempool, state, MakeTransactionRef(std::move(_tx)), false, nullptr, nullptr, true, 0));
    ...
    state = CValidationState();
    {
        ...
        BOOST_CHECK(!AcceptToMemoryPool(mempool, state, MakeTransactionRef(std::move(_tx2)), false, nullptr, nullptr, true, 0));
        BOOST_CHECK_MESSAGE(res, state.GetRejectReason());
    }
    ...
    {
        CTransaction _tx2(tx2);
        BOOST_CHECK(res = AcceptToMemoryPool(mempool, state, MakeTransactionRef(std::move(_tx2)), false, nullptr, nullptr, true, 0));
        BOOST_CHECK_MESSAGE(res, state.GetRejectReason());
    }
