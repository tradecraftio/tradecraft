...
Verify(const CScript& scriptSig, const CScript& scriptPubKey, bool fStrict, ScriptError& err)
{
    ...
    return VerifyScript(scriptSig, scriptPubKey, NULL, fStrict ? SCRIPT_VERIFY_P2SH : SCRIPT_VERIFY_NONE, MutableTransactionSignatureChecker(&txTo, 0, txFrom.vout[0].nValue, txFrom.lock_height), &err);
}
...
BOOST_AUTO_TEST_CASE(AreInputsStandard)
{
    ...
    txFrom.vout[7].scriptPubKey = GetScriptForDestination(CScriptID(nineteenSigops));
    ...
    txFrom.vout[8].scriptPubKey = GetScriptForDestination(CScriptID(twentyoftwenty));
    ...
    txFrom.vout[9].scriptPubKey = GetScriptForDestination(CScriptID(twentyoneSigops));
    ...
    txToStd3.vout[0].scriptPubKey = GetScriptForDestination(key[1].GetPubKey().GetID());
    txToStd3.vout[0].nValue = 1000;
    txToStd3.vin.resize(1);
    txToStd3.vin[0].prevout.n = 7;
    txToStd3.vin[0].prevout.hash = txFrom.GetHash();
    txToStd3.vin[0].scriptSig << vector<unsigned char>(nineteenSigops.begin(), nineteenSigops.end());

    BOOST_CHECK(::AreInputsStandard(txToStd3, coins));
    BOOST_CHECK_EQUAL(GetP2SHSigOpCount(txToStd3, coins), 19U);

    CMutableTransaction txToStd4;
    txToStd4.vout.resize(1);
    txToStd4.vout[0].scriptPubKey = GetScriptForDestination(key[1].GetPubKey().GetID());
    txToStd4.vout[0].nValue = 1000;
    txToStd4.vin.resize(1);
    txToStd4.vin[0].prevout.n = 8;
    txToStd4.vin[0].prevout.hash = txFrom.GetHash();
    txToStd4.vin[0].scriptSig << vector<unsigned char>(twentyoftwenty.begin(), twentyoftwenty.end());

    BOOST_CHECK(::AreInputsStandard(txToStd4, coins));
    BOOST_CHECK_EQUAL(GetP2SHSigOpCount(txToStd4, coins), 20U);

    CMutableTransaction txToNonStd5;
    txToNonStd5.vout.resize(1);
    txToNonStd5.vout[0].scriptPubKey = GetScriptForDestination(key[1].GetPubKey().GetID());
    ...
    txToNonStd5.vin[0].scriptSig << vector<unsigned char>(twentyoneSigops.begin(), twentyoneSigops.end());

    BOOST_CHECK(!::AreInputsStandard(txToNonStd5, coins));
    BOOST_CHECK_EQUAL(GetP2SHSigOpCount(txToNonStd5, coins), 21U);
}
