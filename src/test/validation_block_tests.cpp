...
void BuildChain(const uint256& root, const BlockFinalTxEntry& entry, int height, const unsigned int invalid_rate, const unsigned int branch_rate, const unsigned int max_size, std::vector<std::pair<std::shared_ptr<const CBlock>, bool>>& blocks)
{
    ...
    if (!gen_invalid) {
        ...
        BuildChain(pblock->GetHash(), next_entry, height - 1, invalid_rate, branch_rate, max_size, blocks);
    }

    if (gen_fork) {
        next_entry = entry;
        blocks.push_back(std::make_pair(GoodBlock(root, next_entry), true));
        if (height == 1) {
            next_entry = InitialBlockFinalTxEntry(*pblock);
        }
        BuildChain(blocks.back().first->GetHash(), next_entry, height - 1, invalid_rate, branch_rate, max_size, blocks);
    }
}
...
BOOST_AUTO_TEST_CASE(processnewblock_signals_ordering)
{
    ...
    while (blocks.size() < 50) {
        blocks.clear();
        BuildChain(Params().GenesisBlock().GetHash(), BlockFinalTxEntry(), 100, 15, 10, 500, blocks);
    }
