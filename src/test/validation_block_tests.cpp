    ...
    for (int i = 0; i < 10; i++) {
        threads.emplace_back([&]() {
            ...
            for (auto block : blocks) {
                if (block.second) {
                    bool processed = Assert(m_node.chainman)->ProcessNewBlock(Params(), block.first, true, &ignored);
                    assert(processed);
                }
            }
        });
...
BOOST_AUTO_TEST_CASE(mempool_locks_reorg)
{
    ...
    for (int test_runs = 3; test_runs > 0; --test_runs) {
        ...
        BOOST_CHECK_EQUAL(height, m_node.chainman->ActiveChain().Tip()->nHeight + 1);
        ...
        // Add the txs to the tx pool
        {
            ...
            for (const auto& tx : txs) {
                const MempoolAcceptResult result = m_node.chainman->ProcessTransaction(Params().GetConsensus(), tx);
