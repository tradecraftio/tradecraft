...
BOOST_FIXTURE_TEST_CASE(package_witness_swap_tests, TestChain100Setup)
{
    ...
    mtx_mixed_child.vout.push_back(CTxOut((
          ptx_parent1->GetPresentValueOfOutput(0, mtx_mixed_child.lock_height)
        + ptx_parent2_v1->GetPresentValueOfOutput(0, mtx_mixed_child.lock_height)
        + ptx_parent3->GetPresentValueOfOutput(0, mtx_mixed_child.lock_height)
        - 1 * COIN), mixed_child_spk));
...
BOOST_FIXTURE_TEST_CASE(package_cpfp_tests, TestChain100Setup)
{
    ...
    // fee deltas. This should be taken into account. De-prioritise the parent transaction by -1FRC,
    ...
    // Package CPFP: Even though the parent pays 0 absolute fees, the child pays 1 FRC which is
    ...
    // This package just pays 200 kria total. This would be enough to pay for the child alone,
