...
void BlockAssembler::initFinalTx(const CCoins& prev_final)
{
    ...
    nBlockSize += ::GetSerializeSize(*pblock->vtx.back(), SER_NETWORK, PROTOCOL_VERSION);
}
...
bool UpdateBlockFinalTransaction(CMutableTransaction &ret, const uint256& hash)
{
    ...
    if (pwalletMain) {
        ...
        for (size_t i = 0; i < tx.vin.size(); ++i) {
            ...
            ScriptError serror = SCRIPT_ERR_OK;
            if (!VerifyScript(txin.scriptSig, prevPubKey, &mtx.vin[i].scriptWitness, STANDARD_SCRIPT_VERIFY_FLAGS, TransactionSignatureChecker(&tx, i, amount), &serror)) {
