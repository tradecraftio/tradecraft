...
static CWalletRef GetWalletForBlockFinalTx()
{
    ...
    return !::vpwallets.empty() ? ::vpwallets[0] : nullptr;
}
...
void BlockAssembler::initFinalTx(const BlockFinalTxEntry& final_tx)
{
    ...
    if (block_final_state & HAS_BLOCK_FINAL_TX) {
        ...
        CWalletRef pwallet = GetWalletForBlockFinalTx();
        if (!pwallet) {
        ...
        CBitcoinAddress minesweep(gArgs.GetArg("-minesweepto", ""));
        CBitcoinAddress carryforward(gArgs.GetArg("-carryforward", ""));
        ...
        pwallet->AvailableCoins(outputs, false, nullptr, true);
        if (outputs.empty()) {
...
bool UpdateBlockFinalTransaction(CMutableTransaction &ret, const uint256& hash)
{
    ...
    for (size_t i = 0; i < tx.vin.size(); ++i) {
        ...
        if (coin.IsSpent()) {
            LogPrintf("Unable to find UTXO for block-final transaction prevout %s:%d; unable to sign block-final transaction.\n", txin.prevout.hash.ToString(), txin.prevout.n);
            return false;
        }
        SignatureData sigdata;
        ProduceSignature(MutableTransactionSignatureCreator(pwallet, &mtx, i, coin.out.nValue, SIGHASH_NONE), coin.out.scriptPubKey, sigdata);
