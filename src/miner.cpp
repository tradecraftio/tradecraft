...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    if (VersionBitsState(pindexPrev, chainparams.GetConsensus(), Consensus::DEPLOYMENT_AUXPOW, versionbitscache) == ThresholdState::ACTIVE) {
...
void BlockAssembler::initFinalTx(const BlockFinalTxEntry& final_tx)
{
    ...
    CAmount nTxFees = ::ChainstateActive().CoinsTip().GetValueIn(*pblock->vtx.back())
    ...
    int64_t nTxSigOpsCost = GetTransactionSigOpCost(*pblock->vtx.back(), ::ChainstateActive().CoinsTip(), STANDARD_SCRIPT_VERIFY_FLAGS);
...
void IncrementExtraNonce(CBlock* pblock, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, unsigned int& nExtraNonce, boost::optional<uint256> aux_hash2)
{
    // Update nExtraNonce
    static uint256 hashPrevBlock;
