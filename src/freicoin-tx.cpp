...
static void SetupFreicoinTxArgs(ArgsManager &argsman)
{
    ...
    argsman.AddArg("outdestroy=VALUE", "Add unspendable output to TX", ArgsManager::ALLOW_ANY, OptionsCategory::COMMANDS);
...
static void MutateTxAddOutPubKey(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    if (bSegWit) {
        ...
        // Build a P2WPK script
        scriptPubKey = GetScriptForDestination(WitnessV0ShortHash(0 /* version */, pubkey));
    }
    if (bScriptHash) {
...
static void MutateTxAddOutMultiSig(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    if (bSegWit) {
        ...
        scriptPubKey = GetScriptForDestination(WitnessV0LongHash(0 /* version */, scriptPubKey));
    }
    if (bScriptHash) {
        ...
        for (unsigned int previdx = 0; previdx < prevtxsObj.size(); previdx++) {
...

            {
                ...
                if (prevOut.exists("value")) {
                    newcoin.out.SetReferenceValue(AmountFromValue(prevOut["value"]));
                }
                newcoin.refheight = 0;
                if (prevOut.exists("refheight")) {
                    newcoin.refheight = prevOut["refheight"].get_int();
                }
                newcoin.nHeight = 1;
