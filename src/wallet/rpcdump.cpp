    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"dumpwallet",
                ...
                {
                    {"filename", RPCArg::Type::STR, RPCArg::Optional::NO, "The filename with path (either absolute or relative to freicoind)"},
                },
                RPCResult{
            "{                           (json object)\n"
...
static UniValue ProcessImportLegacy(ImportData& import_data, std::map<CKeyID, CPubKey>& pubkey_map, std::map<CKeyID, CKey>& privkey_map, std::set<CScript>& script_pub_keys, bool& have_solving_data, const UniValue& data, std::vector<CKeyID>& ordered_pubkeys)
{
    ...
    if (witness_script_hex.size()) {
        ...
        import_data.witnessscript = MakeUnique<WitnessV0ScriptEntry>(parsed_witnessscript);
    }
...
static UniValue ProcessImport(CWallet * const pwallet, const UniValue& data, const int64_t timestamp) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    try {
        ...
        pwallet->MarkDirty();
        if (!pwallet->ImportWitnessV0Scripts(import_data.import_witscripts, timestamp)) {
