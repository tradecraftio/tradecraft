...
TransactionError LegacyScriptPubKeyMan::FillPST(PartiallySignedTransaction& pstx, const PrecomputedTransactionData& txdata, int sighash_type, bool sign, bool bip32derivs, int* n_signed) const
{
    if (n_signed) {
        *n_signed = 0;
    }
    for (unsigned int i = 0; i < pstx.tx->vin.size(); ++i) {
        ...
        SignPSTInput(HidingSigningProvider(this, !sign, !bip32derivs), pstx, i, &txdata, sighash_type);
...
TransactionError DescriptorScriptPubKeyMan::FillPST(PartiallySignedTransaction& pstx, const PrecomputedTransactionData& txdata, int sighash_type, bool sign, bool bip32derivs, int* n_signed) const
{
    if (n_signed) {
        *n_signed = 0;
    }
    for (unsigned int i = 0; i < pstx.tx->vin.size(); ++i) {
        ...
        SignPSTInput(HidingSigningProvider(keys.get(), !sign, !bip32derivs), pstx, i, &txdata, sighash_type);
    ...
    for (unsigned int i = 0; i < pstx.tx->vout.size(); ++i) {
        ...
        UpdatePSTOutput(HidingSigningProvider(keys.get(), true, !bip32derivs), pstx, i);
    }
