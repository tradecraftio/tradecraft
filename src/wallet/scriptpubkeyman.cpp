...
const std::unordered_set<CScript, SaltedSipHasher> LegacyScriptPubKeyMan::GetScriptPubKeys() const
{
    ...
    // Now we add witness scripts
    for (const auto& witscript_pair : mapWitnessV0Scripts) {
        const WitnessV0ScriptEntry& entry = witscript_pair.second;
        spks.insert(GetScriptForDestination(entry.GetLongHash()));
        spks.insert(GetScriptForDestination(entry.GetShortHash()));
    }
...
TransactionError DescriptorScriptPubKeyMan::FillPST(PartiallySignedTransaction& pstx, const PrecomputedTransactionData& txdata, int sighash_type, bool sign, bool bip32derivs, int* n_signed, bool finalize) const
{
    ...
    for (unsigned int i = 0; i < pstx.tx->vin.size(); ++i) {
        ...
        SignPSTInput(HidingSigningProvider(keys.get(), !sign, !bip32derivs), pstx, i, &txdata, sighash_type, nullptr, finalize);
