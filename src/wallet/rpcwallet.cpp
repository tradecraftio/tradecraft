...
public:
    ...
    bool operator()(const ScriptHash &scripthash) {
        CScript subscript;
        if (pwallet && pwallet->GetCScript(scripthash, subscript)) {
...
static UniValue bumpfee(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
        return NullUniValue;
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "",
                        {
                            ...
                            {"totalFee", RPCArg::Type::NUM, /* default */ "fallback to 'confTarget'", "Total fee (NOT feerate) to pay, in kria. (DEPRECATED)\n"
            ...
            "                         Must be at least 0.0001 FRC per kB higher than the current transaction fee rate.\n"},
                            {"estimate_mode", RPCArg::Type::STR, /* default */ "UNSET", "The fee estimate mode, must be one of:\n"
            "         \"UNSET\"\n"
            "         \"ECONOMICAL\"\n"
            "         \"CONSERVATIVE\""},
