...
UniValue SendMoney(CWallet* const pwallet, const CCoinControl &coin_control, std::vector<CRecipient> &recipients, int64_t refheight, mapValue_t map_value, bool verbose)
{
    ...
    const bool fCreated = pwallet->CreateTransaction(recipients, refheight, tx, nFeeRequired, nChangePosRet, error, coin_control, fee_calc_out, true);
    if (!fCreated) {
...
static RPCHelpMan addmultisigaddress()
{
    return RPCHelpMan{"addmultisigaddress",
                ...
                "If 'label' is specified, assign address to that label.\n",
                {
                    ...
                    {"keys", RPCArg::Type::ARR, RPCArg::Optional::NO, "The freicoin addresses or hex-encoded public keys",
                        {
...
public:
    ...
    Witnessifier(CWallet *_pwallet, const std::vector<uint256>& branchIn, uint32_t pathIn) : pwallet(_pwallet), entry(std::vector<unsigned char>(), branchIn, pathIn), already_witness(false) { }
    Witnessifier(CWallet *_pwallet, std::vector<uint256>&& branchIn, uint32_t pathIn) : pwallet(_pwallet), entry(std::vector<unsigned char>(), branchIn, pathIn), already_witness(false) { }
...
static UniValue addwitnessaddress(const JSONRPCRequest& request)
{
    ...
    {
        std::string msg = "addwitnessaddress \"address\" (\"proof\")\n"
    ...
    return RPCHelpMan{"listtransactions",
                ...
                RPCResult{
                    RPCResult::Type::ARR, "", "",
                    {
                        ...
                        {
                            {RPCResult::Type::BOOL, "involvesWatchonly", "Only returns true if imported addresses were involved in transaction."},
                            {RPCResult::Type::STR, "address", "The freicoin address of the transaction."},
                ...
                RPCResult{
                    ...
                    TransactionDescriptionString()),
                    {
                        {RPCResult::Type::ARR, "details", "",
                        {
                            ...
                            {
                                {RPCResult::Type::BOOL, "involvesWatchonly", "Only returns true if imported addresses were involved in transaction."},
                                {RPCResult::Type::STR, "address", "The freicoin address involved in the transaction."},
                                {RPCResult::Type::STR, "category", "The transaction category.\n"
                                    "\"send\"                  Transactions sent.\n"
                                    "\"receive\"               Non-coinbase transactions received.\n"
                                    "\"generate\"              Coinbase transactions received with more than 100 confirmations.\n"
...
static RPCHelpMan listunspent()
{
    return RPCHelpMan{
                ...
                "Optionally filter to only include txouts paid to specified addresses.\n",
                {
                    ...
                    {"addresses", RPCArg::Type::ARR, /* default */ "empty array", "The freicoin addresses to filter",
                        {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[4].isNull()) {
        ...
        if (options.exists("atheight"))
            atheight = options["height"].get_int();
    ...
    std::vector<COutput> vecOutputs;
    {
        ...
        LOCK(pwallet->cs_wallet);
        pwallet->AvailableCoins(vecOutputs, atheight, !include_unsafe, &cctl, nMinimumAmount, nMaximumAmount, nMinimumSumAmount, nMaximumCount);
    }
...
{
    bool want_pst = method_name == "pstbumpfee";
    ...
    "         \"" + FeeModes("\"\n\"") + "\""},
        ...
        RPCResult{
            RPCResult::Type::OBJ, "", "", Cat(Cat<std::vector<RPCResult>>(
            {
                {RPCResult::Type::STR, "pst", "The hex-encoded unsigned PST of the new transaction." + std::string(want_pst ? "" : " Only returned when wallet private keys are disabled. (DEPRECATED)")},
            },
            want_pst ? std::vector<RPCResult>{} : std::vector<RPCResult>{{RPCResult::Type::STR_HEX, "txid", "The id of the new transaction. Only returned when wallet private keys are enabled."}}
            ),
            {
    ...
    "\nBump the fee, get the new transaction\'s" + std::string(want_pst ? "pst" : "txid") + "\n" +
        ...
        [want_pst](const RPCHelpMan& self, const JSONRPCRequest& request) mutable -> UniValue
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    if (!wallet) return NullUniValue;
    CWallet* const pwallet = wallet.get();

    if (pwallet->IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS) && !want_pst) {
        if (!pwallet->chain().rpcEnableDeprecated("bumpfee")) {
            throw JSONRPCError(RPC_METHOD_DEPRECATED, "Using bumpfee with wallets that have private keys disabled is deprecated. Use pstbumpfee instead or restart freicoind with -deprecatedrpc=bumpfee. This functionality will be removed in v22");
        }
        want_pst = true;
    }
    ...
    // otherwise return the hex-encoded unsigned PST of the new transaction.
    if (!want_pst) {
        ...
        bool complete = false;
        const TransactionError err = pwallet->FillPST(pstx, complete, SIGHASH_ALL, false /* sign */, true /* bip32derivs */);
...
static RPCHelpMan pstbumpfee() { return bumpfee_helper("pstbumpfee"); }
    ...
    return RPCHelpMan{"send",
        ...
        "\nSend a transaction.\n",
        {
            ...
            {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "",
                {
                    ...
                    {"change_address", RPCArg::Type::STR_HEX, /* default */ "pool address", "The freicoin address to receive the change"},
            ...
            "       \"" + FeeModes("\"\n\"") + "\""},
                    ...
                    {"height", RPCArg::Type::NUM, /* default */ "0", "Raw locktime. Non-0 value also locktime-activates inputs"},
                    {"lock_unspents", RPCArg::Type::BOOL, /* default */ "false", "Lock selected unspent outputs"},
                    {"pst", RPCArg::Type::BOOL,  /* default */ "automatic", "Always return a PST, implies add_to_wallet=false."},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            int current_height = -1;
            {
                ...
                current_height = tip_height.get();
            }

            CAmount fee;
            int change_position;
            CMutableTransaction rawTx = ConstructTransaction(options["inputs"], request.params[0], options["locktime"], options["lockheight"], current_height);
            CCoinControl coin_control;
            ...
            bool complete = true;
            const TransactionError err = pwallet->FillPST(pstx, complete, SIGHASH_ALL, true, false);
...
static RPCHelpMan walletprocesspst()
{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    bool complete = true;
    const TransactionError err = pwallet->FillPST(pstx, complete, nHashType, sign, bip32derivs);
    if (err != TransactionError::OK) {
        throw JSONRPCTransactionError(err);
    }
...
static RPCHelpMan walletcreatefundedpst()
{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    CAmount fee;
    int change_position;
    CMutableTransaction rawTx = ConstructTransaction(request.params[0], request.params[1], request.params[2], request.params[3], current_height);
    CCoinControl coin_control;
    // Automatically select coins, unless at least one is manually selected. Can
    // be overridden by options.add_inputs.
    coin_control.m_add_inputs = rawTx.vin.size() == 0;
    ...
    bool complete = true;
    const TransactionError err = pwallet->FillPST(pstx, complete, 1, false, bip32derivs);
    if (err != TransactionError::OK) {
        throw JSONRPCTransactionError(err);
    }
...
{ //  category              name                                actor (function)                argNames
    ...
    { "wallet",             "pstbumpfee",                      &pstbumpfee,                   {"txid", "options"} },
    ...
    { "wallet",             "sendmany",                         &sendmany,                      {"dummy","amounts","minconf","comment","subtractfeefrom","conf_target","estimate_mode","fee_rate","verbose"} },
    { "wallet",             "sendtoaddress",                    &sendtoaddress,                 {"address","amount","comment","comment_to","subtractfeefromamount","conf_target","estimate_mode","avoid_reuse","fee_rate","verbose"} },
    ...
    { "wallet",             "walletcreatefundedpst",           &walletcreatefundedpst,        {"inputs","outputs","locktime","lockheight","options","bip32derivs"} },
