...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress \"freicoinaddress\" amount ( \"comment\" \"comment-to\" subtractfeefromamount )\n"
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            ...
            "2. \"tofreicoinaddress\"  (string, required) The freicoin address to send funds to.\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    ...
    CAmount nFee = (wtx.IsFromMe(filter) ? wtx.GetValueOut() - value_in : 0);
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs) {
        ...
        entry.push_back(Pair("value", ValueFromAmount(out.tx->vout[out.i].GetReferenceValue())));
        entry.push_back(Pair("amount", ValueFromAmount(out.tx->GetPresentValueOfOutput(out.i, next_height))));
    ...
    if (params.size() > 1) {
      if (params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_PARAMETER, "changeAddress must be a valid freicoin address");
