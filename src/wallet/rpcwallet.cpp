...
static UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    } else if (output_type == OutputType::BECH32 && !is_witness_enabled) {
...
static CTransactionRef SendMoney(interfaces::Chain::Lock& locked_chain, CWallet * const pwallet, const CTxDestination &address, CAmount nValue, int64_t refheight, bool fSubtractFeeFromAmount, const CCoinControl& coin_control, mapValue_t mapValue)
{
    ...
    CTransactionRef tx;
    if (!pwallet->CreateTransaction(locked_chain, vecSend, refheight, tx, reservekey, nFeeRequired, nChangePosRet, strError, coin_control)) {
...
static UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    CTransactionRef tx = SendMoney(*locked_chain, pwallet, dest, nAmount, -1, fSubtractFeeFromAmount, coin_control, std::move(mapValue));
...
static UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    CTransactionRef tx;
    bool fCreated = pwallet->CreateTransaction(*locked_chain, vecSend, -1, tx, keyChange, nFeeRequired, nChangePosRet, strFailReason, coin_control);
    if (!fCreated)
...
static UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    std::vector<COutput> vecOutputs;
    {
        auto locked_chain = pwallet->chain().lock();
        LOCK(pwallet->cs_wallet);
        pwallet->AvailableCoins(*locked_chain, chainActive.Height() + 1, vecOutputs, !include_unsafe, nullptr, nMinimumAmount, nMaximumAmount, nMinimumSumAmount, nMaximumCount, nMinDepth, nMaxDepth);
    }
