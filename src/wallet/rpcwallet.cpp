...
UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    ...
    BOOST_FOREACH(const PAIRTYPE(CFreicoinAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
...
static void SendMoney(const CTxDestination &address, CAmount nValue, int32_t refheight, bool fSubtractFeeFromAmount, CWalletTx& wtxNew)
{
    ...
    if (!pwalletMain->CreateTransaction(vecSend, refheight, wtxNew, reservekey, nFeeRequired, nChangePosRet, strError)) {
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    SendMoney(address.Get(), nAmount, -1, fSubtractFeeFromAmount, wtx);
        ...
        BOOST_FOREACH(CTxDestination address, grouping)
        {
            ...
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                if (pwalletMain->mapAddressBook.find(CFreicoinAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CFreicoinAddress(address).Get())->second.name);
            }
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    SendMoney(address.Get(), nAmount, -1, false, wtx);
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    BOOST_FOREACH(const string& name_, keys)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Freicoin address: ")+name_);
    ...
    bool fCreated = pwalletMain->CreateTransaction(vecSend, -1, wtx, keyChange, nFeeRequired, nChangePosRet, strFailReason);
    if (!fCreated)
...
public:
    ...
    bool operator()(const CKeyID &keyID) {
        ...
        if (pwalletMain) {
            ...
            uint256 h256;
            CHash256().Write(&innerscript[0], innerscript.size()).Finalize(h256.begin());
...
UniValue bumpfee(const JSONRPCRequest& request)
{
    ...
    if (totalFee > 0) {
        ...
        // add 1 kria to the result, because it may have been rounded down.
