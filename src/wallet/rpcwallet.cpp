...
static CTransactionRef SendMoney(interfaces::Chain::Lock& locked_chain, CWallet * const pwallet, const CTxDestination &address, CAmount nValue, int64_t refheight, bool fSubtractFeeFromAmount, const CCoinControl& coin_control, mapValue_t mapValue)
{
    ...
    CTransactionRef tx;
    if (!pwallet->CreateTransaction(locked_chain, vecSend, refheight, tx, nFeeRequired, nChangePosRet, strError, coin_control)) {
...
static UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    CTransactionRef tx = SendMoney(*locked_chain, pwallet, dest, nAmount, -1 /* refheight */, fSubtractFeeFromAmount, coin_control, std::move(mapValue));
...
static UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    CTransactionRef tx;
    bool fCreated = pwallet->CreateTransaction(*locked_chain, vecSend, -1 /* refheight */, tx, nFeeRequired, nChangePosRet, strFailReason, coin_control);
    if (!fCreated)
...
public:
    ...
    bool operator()(const ScriptHash &scripthash) {
        if (pwallet) {
            ...
            std::unique_ptr<SigningProvider> provider = pwallet->GetSolvingProvider(script);
            CScript subscript;
            if (provider && provider->GetCScript(scripthash, subscript)) {
...
static UniValue bumpfee(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
        return NullUniValue;
            ...
            RPCHelpMan{"bumpfee",
                "\nBumps the fee of an RBF transaction T, replacing it with a new transaction B.\n"
                "An RBF transaction with the given txid must be in the wallet.\n"
            ...
            "         \"CONSERVATIVE\""},
                ...
                RPCResult{
                    RPCResult::Type::OBJ, "", "", {
                        {RPCResult::Type::STR, "pst", "The base64-encoded unsigned PST of the new transaction. Only returned when wallet private keys are disabled."},
    ...
    // otherwise return the base64-encoded unsigned PST of the new transaction.
    if (!pwallet->IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS)) {
        ...
        return NullUniValue;
            ...
            "                                                         getaddressinfo output fields for the embedded address, excluding metadata (timestamp, hdkeypath,\n"
                            ...
                            "as an array to keep the API stable if multiple labels are enabled in the future.",
                        {
                            ...
                            {RPCResult::Type::OBJ, "", "label data, DEPRECATED, will be removed in v21. To re-enable, launch freicoind with `-deprecatedrpc=labelspurpose`",
                            {
...
UniValue walletcreatefundedpst(const JSONRPCRequest& request)
{
    ...
    int current_height = -1;
    {
        auto locked_chain = pwallet->chain().lock();
        if (!locked_chain) {
            throw JSONRPCError(RPC_INTERNAL_ERROR, "Unable to get a locked view into the current chain.  This should never happen.");
        }
        const Optional<int> tip_height = locked_chain->getHeight();
        if (!tip_height) {
