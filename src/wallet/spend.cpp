...
{
    // nVersion + nLockTime + lock_height + input count + output count
    ...
    for (const auto& txo : tx.vout) weight += GetSerializeSize(txo, SER_NETWORK, PROTOCOL_VERSION) * WITNESS_SCALE_FACTOR;
        ...
        bool sign)
{
    ...
    if (refheight < 0 || refheight > std::numeric_limits<uint32_t>::max()) {
        return util::Error{_("Reference height out of range")};
    }
    const uint32_t lock_height = static_cast<uint32_t>(refheight);

    auto res = CreateTransactionInternal(wallet, vecSend, lock_height, change_pos, coin_control, sign);
    ...
    if (txr_ungrouped.fee > 0 /* 0 means non-functional fee rate estimation */ && wallet.m_max_aps_fee > -1 && !coin_control.m_avoid_partial_spends) {
        ...
        auto txr_grouped = CreateTransactionInternal(wallet, vecSend, lock_height, change_pos, tmp_cc, sign);
...
bool FundTransaction(CWallet& wallet, CMutableTransaction& tx, CAmount& nFeeRet, int& nChangePosInOut, bilingual_str& error, bool lockUnspents, const std::set<int>& setSubtractFeeFromOutputs, CCoinControl coinControl)
{
    ...
    for (size_t idx = 0; idx < tx.vout.size(); idx++) {
        ...
        CRecipient recipient = {dest, txOut.GetReferenceValue(), setSubtractFeeFromOutputs.count(idx) == 1};
    ...
    auto res = CreateTransaction(wallet, vecSend, tx.lock_height ? static_cast<int64_t>(tx.lock_height) : -1LL, nChangePosInOut, coinControl, false);
    if (!res) {
    ...
    // Copy lock_height from new transaction
    tx.lock_height = tx_new->lock_height;
