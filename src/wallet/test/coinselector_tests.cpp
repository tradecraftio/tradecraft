...
static void add_coin(const CAmount& nValue, int nInput, SelectionResult& result, CAmount fee, CAmount long_term_fee)
{
    ...
    tx.nLockTime = nextLockTime++;        // so all transactions get different hashes
    tx.lock_height = 1;
    std::shared_ptr<COutput> coin = std::make_shared<COutput>(tx.lock_height, COutPoint(tx.GetHash(), nInput), SpentOutput{tx.vout.at(nInput), /*refheight=*/ 1}, /*depth=*/ 1, /*input_bytes=*/ 148, /*spendable=*/ true, /*solvable=*/ true, /*safe=*/ true, /*time=*/ 0, /*from_me=*/ false, fee);
    OutputGroup group;
...
static void add_coin(CoinsResult& available_coins, CWallet& wallet, const CAmount& nValue, CFeeRate feerate = CFeeRate(0), int nAge = 6*24, bool fIsFromMe = false, int nInput =0, bool spendable = false, int custom_size = 0)
{
    ...
    available_coins.Add(OutputType::BECH32, {tx.lock_height, COutPoint(wtx.GetHash(), nInput), SpentOutput{txout, /*refheight=*/1}, nAge, custom_size == 0 ? CalculateMaximumSignedInputSize(txout, &wallet, /*coin_control=*/nullptr) : custom_size, /*spendable=*/ true, /*solvable=*/ true, /*safe=*/ true, wtx.GetTxTime(), fIsFromMe, feerate});
}
