...
static UniValue FinishTransaction(const std::shared_ptr<CWallet> pwallet, const UniValue& options, const CMutableTransaction& rawTx)
{
    ...
    const auto err{pwallet->FillPST(pstx, complete, SIGHASH_DEFAULT, /*sign=*/true, /*bip32derivs=*/false)};
    if (err) {
        throw JSONRPCPSTError(*err);
    }
...
UniValue SendMoney(CWallet& wallet, const CCoinControl &coin_control, std::vector<CRecipient> &recipients, std::optional<uint32_t> refheight, mapValue_t map_value, bool verbose)
{
    ...
    // Send
    auto res = CreateTransaction(wallet, recipients, refheight, /*change_pos=*/std::nullopt, coin_control, true);
    if (!res) {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    return SendMoney(*pwallet, coin_control, recipients, /*refheight=*/std::nullopt, mapValue, verbose);
},
    };
}
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    return SendMoney(*pwallet, coin_control, recipients, /*refheight=*/std::nullopt, std::move(mapValue), verbose);
},
    };
}
                ...
                "Only pay-to-pubkey, multisig, and P2SH versions thereof are currently supported for watch-only\n",
                {
                    ...
                    {"options", RPCArg::Type::OBJ_NAMED_PARAMS, RPCArg::Optional::OMITTED, "For backward compatibility: passing in a true instead of an object will result in {\"includeWatching\":true}",
                        Cat<std::vector<RPCArg>>(
                        {
                            ...
                            {"change_type", RPCArg::Type::STR, RPCArg::DefaultHint{"set by -changetype"}, "The output type to use. Only valid if changeAddress is not specified. Options are \"legacy\", \"bech32\", or \"bech32m\"."},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    for (const auto& tx_out : tx.vout) {
        ...
        destinations.emplace_back(dest, tx_out.GetReferenceValue());
    }
        ...
        [want_pst](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!want_pst) {
        ...
        bool complete = false;
        const auto err{pwallet->FillPST(pstx, complete, SIGHASH_DEFAULT, /*sign=*/false, /*bip32derivs=*/true)};
        CHECK_NONFATAL(!err);
        ...
        "\nSend a transaction.\n",
        {
            ...
            {"options", RPCArg::Type::OBJ_NAMED_PARAMS, RPCArg::Optional::OMITTED, "",
                Cat<std::vector<RPCArg>>(
                {
                    ...
                    {"change_type", RPCArg::Type::STR, RPCArg::DefaultHint{"set by -changetype"}, "The output type to use. Only valid if change_address is not specified. Options are \"legacy\", \"bech32\" or \"bech32m\"."},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    const auto err{wallet.FillPST(pstx, complete, nHashType, sign, bip32derivs, nullptr, finalize)};
    if (err) {
        throw JSONRPCPSTError(*err);
    }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Make a blank pst
    PartiallySignedTransaction pstx(CMutableTransaction(*txr.tx));

    // Fill transaction with out data but don't sign
    bool bip32derivs = request.params[5].isNull() ? true : request.params[5].get_bool();
    bool complete = true;
    const auto err{wallet.FillPST(pstx, complete, 1, /*sign=*/false, /*bip32derivs=*/bip32derivs)};
    if (err) {
        throw JSONRPCPSTError(*err);
    }
