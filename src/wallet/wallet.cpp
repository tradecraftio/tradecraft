...
/**
 * Fees smaller than this (in kria) are considered zero fee (for transaction creation)
...
struct CompareValueOnly
{
    ...
    {
        return t1.adjusted < t2.adjusted;
    }
};
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CFreicoinAddress(CScriptID(redeemScript)).ToString();
...
bool CWallet::AddWitnessV0Script(const std::vector<unsigned char>& script)
{
    ...
    return CWalletDB(*dbw).WriteWitnessV0Script(shorthash, script);
}
...
CAmount CWallet::GetBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->IsTrusted())
                nTotal += GetTimeAdjustedValue(pcoin->GetAvailableCredit(), next_height - (int)pcoin->tx->lock_height);
        }
    }
...
CAmount CWallet::GetImmatureBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        {
            const CWalletTx* pcoin = &(*it).second;
            nTotal += GetTimeAdjustedValue(pcoin->GetImmatureCredit(), next_height - (int)pcoin->tx->lock_height);
        }
    }
    return nTotal;
}
...
CAmount CWallet::GetWatchOnlyBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            const CWalletTx* pcoin = &(*it).second;
            if (pcoin->IsTrusted())
                nTotal += GetTimeAdjustedValue(pcoin->GetAvailableWatchOnlyCredit(), next_height - (int)pcoin->tx->lock_height);
        }
    }
...
CAmount CWallet::GetImmatureWatchOnlyBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        {
            const CWalletTx* pcoin = &(*it).second;
            nTotal += GetTimeAdjustedValue(pcoin->GetImmatureWatchOnlyCredit(), next_height - (int)pcoin->tx->lock_height);
        }
    }
    return nTotal;
}

// GetBalance sums up all available unspent TxOuts AND applies demurrage.
// GetBalance and GetLegacyBalance should NOT return the same number because
// demurrage is not applied in account views, even the '*' aggregate-account
// view.
    ...
    for (const auto& entry : mapWallet) {
        ...
        for (const CTxOut& out : wtx.tx->vout) {
            if (outgoing && IsChange(out)) {
                debit -= out.GetReferenceValue();
            } else if (IsMine(out) & filter && depth >= minDepth && (!account || *account == GetAccountName(out.scriptPubKey))) {
                balance += out.GetReferenceValue();
            }
        }
...

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                CAmount amount = pcoin->tx->GetPresentValueOfOutput(i, height);
                ...
                vCoins.push_back(COutput(pcoin, i, nDepth, amount, fSpendableIn, fSolvableIn, safeTx));
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    AvailableCoins(next_height, availableCoins);
            ...
            if (depth >= 0 && output.n < it->second.tx->vout.size() &&
                ...
                if (ExtractDestination(FindNonChangeParentOutput(*it->second.tx, output.n).scriptPubKey, address)) {
                    result[address].emplace_back(
                        &it->second, output.n, depth, it->second.tx->GetPresentValueOfOutput(output.n, next_height), true /* spendable */, true /* solvable */, false /* safe */);
                }
            }
        }
    }
...
bool CWallet::SelectCoinsMinConf(const CAmount& nTargetValue, uint32_t height, const int nConfMine, const int nConfTheirs, const uint64_t nMaxAncestors, std::vector<COutput> vCoins,
                                 std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet) const
{
    ...
    for (const COutput &output : vCoins)
    {
        ...
        CInputCoin coin = CInputCoin(pcoin, i, height);

        if (coin.adjusted == nTargetValue)
        {
            setCoinsRet.insert(coin);
            nValueRet += coin.adjusted;
            return true;
        }
        else if (coin.adjusted < nTargetValue + MIN_CHANGE)
        {
            vValue.push_back(coin);
            nTotalLower += coin.adjusted;
        }
        else if (!coinLowestLarger || coin.adjusted < coinLowestLarger->adjusted)
        {
    ...
    if (nTotalLower < nTargetValue)
    {
        ...
        nValueRet += coinLowestLarger->adjusted;
        return true;
    }
    ...
    if (coinLowestLarger &&
        ((nBest != nTargetValue && nBest < nTargetValue + MIN_CHANGE) || coinLowestLarger->adjusted <= nBest))
    {
        setCoinsRet.insert(coinLowestLarger.get());
        nValueRet += coinLowestLarger->adjusted;
    }
        ...
        if (LogAcceptCategory(BCLog::SELECTCOINS)) {
            ...
            for (unsigned int i = 0; i < vValue.size(); i++) {
                if (vfBest[i]) {
                    LogPrint(BCLog::SELECTCOINS, "%s ", FormatMoney(vValue[i].adjusted));
                }
            }
...
bool CWallet::SelectCoins(const std::vector<COutput>& vAvailableCoins, const CAmount& nTargetValue, uint32_t height, std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl) const
{
    ...
    if (coinControl && coinControl->HasSelected() && !coinControl->fAllowOtherInputs)
    {
        for (const COutput& out : vCoins)
        {
            ...
            setCoinsRet.insert(CInputCoin(out.tx, out.i, height, out.adjusted));
        }
    ...
    for (const COutPoint& outpoint : vPresetInputs)
    {
        ...
        if (it != mapWallet.end())
        {
            ...
            if (pcoin->tx->lock_height > height)
                return false;
            CAmount amount = pcoin->tx->GetPresentValueOfOutput(outpoint.n, height);
            nValueFromPresetInputs += amount;
            setPresetCoins.insert(CInputCoin(pcoin, outpoint.n, height, amount));
        } else
    ...
    {
        if (setPresetCoins.count(CInputCoin(it->tx, it->i, height, it->adjusted)))
            it = vCoins.erase(it);
        else
            ++it;
    }
    ...
    bool res = nTargetValue <= nValueFromPresetInputs ||
        SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, height, 1, 6, 0, vCoins, setCoinsRet, nValueRet) ||
        SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, height, 1, 1, 0, vCoins, setCoinsRet, nValueRet) ||
        (bSpendZeroConfChange && SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, height, 0, 1, 2, vCoins, setCoinsRet, nValueRet)) ||
        (bSpendZeroConfChange && SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, height, 0, 1, std::min((size_t)4, nMaxChainLength/3), vCoins, setCoinsRet, nValueRet)) ||
        (bSpendZeroConfChange && SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, height, 0, 1, nMaxChainLength/2, vCoins, setCoinsRet, nValueRet)) ||
        (bSpendZeroConfChange && SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, height, 0, 1, nMaxChainLength, vCoins, setCoinsRet, nValueRet)) ||
        (bSpendZeroConfChange && !fRejectLongChains && SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, height, 0, 1, std::numeric_limits<uint64_t>::max(), vCoins, setCoinsRet, nValueRet));
...
bool CWallet::FundTransaction(CMutableTransaction& tx, CAmount& nFeeRet, int& nChangePosInOut, std::string& strFailReason, bool lockUnspents, const std::set<int>& setSubtractFeeFromOutputs, CCoinControl coinControl)
{
    ...
    CWalletTx wtx;
    if (!CreateTransaction(vecSend, tx.lock_height ? tx.lock_height : -1, wtx, reservekey, nFeeRet, nChangePosInOut, strFailReason, coinControl, false)) {
        return false;
    }
    ...
    for (unsigned int idx = 0; idx < tx.vout.size(); idx++)
        tx.vout[idx].SetReferenceValue(wtx.tx->vout[idx].GetReferenceValue());
...
bool CWallet::CreateTransaction(const std::vector<CRecipient>& vecSend, int64_t refheight, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet,
    ...
    unsigned int nBytes;
    {
        ...
        }
        {
            ...
            while (true)
            {
                ...
                if (pick_new_inputs) {
                    nValueIn = 0;
                    setCoins.clear();
                    if (!SelectCoins(vAvailableCoins, nValueToSelect, txNew.lock_height, setCoins, nValueIn, &coin_control))
                    {
                ...
                //
                // We use the highest possible value in that range (maxint-1)
        ...
        if (sign)
        {
            ...
            for (const auto& coin : setCoins)
            {
                ...
                if (!ProduceSignature(TransactionSignatureCreator(this, &txNewConst, nIn, coin.txout.GetReferenceValue(), coin.refheight, SIGHASH_ALL), scriptPubKey, sigdata))
                {
                    strFailReason = _("Signing transaction failed");
                    return false;
                } else {
...
CAmount CWallet::GetMinimumFee(unsigned int nTxBytes, const CCoinControl& coin_control, const CTxMemPool& pool, const CBlockPolicyEstimator& estimator, FeeCalculation *feeCalc)
{
    ...
    */
    ...
    if (coin_control.m_feerate) { // 1.
        ...
        // By default estimates are economical because the user can use transaction replacement
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CFreicoinAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CFreicoinAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = CFreicoinAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(CFreicoinAddress(address).ToString());
    return CWalletDB(*dbw).EraseName(CFreicoinAddress(address).ToString());
}
...

    {
        ...
        for (const auto& walletEntry : mapWallet)
        {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++)
            {
                ...
                CAmount n = IsSpent(walletEntry.first, i) ? 0 : pcoin->tx->vout[i].GetReferenceValue();
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CFreicoinAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CFreicoinAddress(dest).ToString(), key);
}
