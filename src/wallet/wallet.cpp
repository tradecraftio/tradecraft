    ...
    CAmount nTotal = 0;
    {
        ...
        for (const auto& entry : mapWallet)
        {
            ...
            if (pcoin->IsTrusted(*locked_chain) && pcoin->GetDepthInMainChain(*locked_chain) >= min_depth) {
                nTotal += GetTimeAdjustedValue(pcoin->GetAvailableCredit(*locked_chain, true, filter), next_height - (int)pcoin->tx->lock_height);
            }
        }
    }
    ...
    CAmount nTotal = 0;
    {
        ...
        for (const auto& entry : mapWallet)
        {
            ...
            if (!pcoin->IsTrusted(*locked_chain) && pcoin->GetDepthInMainChain(*locked_chain) == 0 && pcoin->InMempool())
                nTotal += GetTimeAdjustedValue(pcoin->GetAvailableCredit(*locked_chain), next_height - (int)pcoin->tx->lock_height);
        }
    }
    return nTotal;
}
...
CAmount CWallet::GetImmatureBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        {
            const CWalletTx* pcoin = &entry.second;
            nTotal += GetTimeAdjustedValue(pcoin->GetImmatureCredit(*locked_chain), next_height - (int)pcoin->tx->lock_height);
        }
    }
    return nTotal;
}
...
CAmount CWallet::GetUnconfirmedWatchOnlyBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        for (const auto& entry : mapWallet)
        {
            const CWalletTx* pcoin = &entry.second;
            if (!pcoin->IsTrusted(*locked_chain) && pcoin->GetDepthInMainChain(*locked_chain) == 0 && pcoin->InMempool())
                nTotal += GetTimeAdjustedValue(pcoin->GetAvailableCredit(*locked_chain, true, ISMINE_WATCH_ONLY), next_height - (int)pcoin->tx->lock_height);
        }
    }
    return nTotal;
}
...
CAmount CWallet::GetImmatureWatchOnlyBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        {
            const CWalletTx* pcoin = &entry.second;
            nTotal += GetTimeAdjustedValue(pcoin->GetImmatureWatchOnlyCredit(*locked_chain), next_height - (int)pcoin->tx->lock_height);
        }
    }
    return nTotal;
}
...
// GetBalance and GetLegacyBalance should NOT return the same number.
...
CAmount CWallet::GetAvailableBalance(const CCoinControl* coinControl) const
{
    ...
    AvailableCoins(*locked_chain, chainActive.Height() + 1, vCoins, true, coinControl);
...
void CWallet::AvailableCoins(interfaces::Chain::Lock& locked_chain, uint32_t height, std::vector<COutput> &vCoins, bool fOnlySafe, const CCoinControl *coinControl, const CAmount &nMinimumAmount, const CAmount &nMaximumAmount, const CAmount &nMinimumSumAmount, const uint64_t nMaximumCount, const int nMinDepth, const int nMaxDepth) const
{
    ...
    for (const auto& entry : mapWallet)
    {
        ...
        for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
            CAmount adjusted_value = pcoin->tx->GetPresentValueOfOutput(i, height);
            ...
            vCoins.push_back(COutput(height, adjusted_value, pcoin, i, nDepth, spendable, solvable, safeTx, (coinControl && coinControl->fAllowWatchOnly)));
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins(interfaces::Chain::Lock& locked_chain) const
{
    ...
    const int next_height = locked_chain.getHeight().get() + 1;
    AvailableCoins(locked_chain, next_height, availableCoins);
...
bool CWallet::SelectCoins(const std::vector<COutput>& vAvailableCoins, const CAmount& nTargetValue, uint32_t height, std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet, const CCoinControl& coin_control, CoinSelectionParams& coin_selection_params, bool& bnb_used) const
{
    ...
    for (const COutPoint& outpoint : vPresetInputs)
    {
        ...
        if (it != mapWallet.end())
        {
            ...
            CAmount adjusted = pcoin->tx->GetPresentValueOfOutput(outpoint.n, height);
            nValueFromPresetInputs += adjusted;
            setPresetCoins.insert(CInputCoin(height, adjusted, pcoin->tx, outpoint.n));
        } else
    ...
    bool res = nTargetValue <= nValueFromPresetInputs ||
        SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, CoinEligibilityFilter(height, 1, 6, 0), groups, setCoinsRet, nValueRet, coin_selection_params, bnb_used) ||
        SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, CoinEligibilityFilter(height, 1, 1, 0), groups, setCoinsRet, nValueRet, coin_selection_params, bnb_used) ||
        (m_spend_zero_conf_change && SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, CoinEligibilityFilter(height, 0, 1, 2), groups, setCoinsRet, nValueRet, coin_selection_params, bnb_used)) ||
        (m_spend_zero_conf_change && SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, CoinEligibilityFilter(height, 0, 1, std::min((size_t)4, max_ancestors/3), std::min((size_t)4, max_descendants/3)), groups, setCoinsRet, nValueRet, coin_selection_params, bnb_used)) ||
        (m_spend_zero_conf_change && SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, CoinEligibilityFilter(height, 0, 1, max_ancestors/2, max_descendants/2), groups, setCoinsRet, nValueRet, coin_selection_params, bnb_used)) ||
        (m_spend_zero_conf_change && SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, CoinEligibilityFilter(height, 0, 1, max_ancestors-1, max_descendants-1), groups, setCoinsRet, nValueRet, coin_selection_params, bnb_used)) ||
        (m_spend_zero_conf_change && !fRejectLongChains && SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, CoinEligibilityFilter(height, 0, 1, std::numeric_limits<uint64_t>::max()), groups, setCoinsRet, nValueRet, coin_selection_params, bnb_used));
...
bool CWallet::FundTransaction(CMutableTransaction& tx, CAmount& nFeeRet, int& nChangePosInOut, std::string& strFailReason, bool lockUnspents, const std::set<int>& setSubtractFeeFromOutputs, CCoinControl coinControl)
{
    ...
    if (!CreateTransaction(*locked_chain, vecSend, tx.lock_height ? tx.lock_height : -1, tx_new, reservekey, nFeeRet, nChangePosInOut, strFailReason, coinControl, false)) {
        return false;
    }
...
bool CWallet::CreateTransaction(interfaces::Chain::Lock& locked_chain, const std::vector<CRecipient>& vecSend, int64_t refheight, CTransactionRef& tx, CReserveKey& reservekey, CAmount& nFeeRet,
                         int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    int nBytes;
    {
        ...
        }
        {
            ...
            AvailableCoins(*locked_chain, refheight, vAvailableCoins, true, &coin_control);
...

    {
        ...
        for (const auto& walletEntry : mapWallet)
        {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++)
            {
                ...
                CAmount n = IsSpent(locked_chain, walletEntry.first, i) ? 0 : pcoin->tx->vout[i].GetReferenceValue();
