...
void CWallet::AvailableCoins(interfaces::Chain::Lock& locked_chain, std::vector<COutput>& vCoins, bool fOnlySafe, const CCoinControl* coinControl, const CAmount& nMinimumAmount, const CAmount& nMaximumAmount, const CAmount& nMinimumSumAmount, const uint64_t nMaximumCount) const
{
    ...
    for (const auto& entry : mapWallet)
    {
        ...
        if (wtx.tx->lock_height > refheight)
            continue;
            ...
            for (auto it = group.m_outputs.begin(); it != group.m_outputs.end(); ) {
                ...
                if (effective_value > 0) {
                    ...
                    if (coin_selection_params.m_subtract_fee_outputs) {
                        group.effective_value += coin.adjusted;
                    } else {
...
bool CWallet::SelectCoins(const std::vector<COutput>& vAvailableCoins, const CAmount& nTargetValue, uint32_t height, std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet, const CCoinControl& coin_control, CoinSelectionParams& coin_selection_params, bool& bnb_used) const
{
    ...
    for (const COutPoint& outpoint : vPresetInputs)
    {
        ...
        if (it != mapWallet.end())
        {
            ...
            coin.effective_value = coin.adjusted - coin_selection_params.effective_fee.GetFee(coin.m_input_bytes);
            if (coin_selection_params.use_bnb) {
...
bool CWallet::FundTransaction(CMutableTransaction& tx, CAmount& nFeeRet, int& nChangePosInOut, std::string& strFailReason, bool lockUnspents, const std::set<int>& setSubtractFeeFromOutputs, CCoinControl coinControl)
{
    ...
    if (!CreateTransaction(*locked_chain, vecSend, tx.lock_height ? tx.lock_height : -1, tx_new, nFeeRet, nChangePosInOut, strFailReason, coinControl, false)) {
        return false;
    }
...
bool CWallet::CreateTransaction(interfaces::Chain::Lock& locked_chain, const std::vector<CRecipient>& vecSend, int64_t refheight, CTransactionRef& tx, CAmount& nFeeRet,
