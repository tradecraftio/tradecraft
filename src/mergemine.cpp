...
static UniValue stratum_mining_aux_notify(AuxWorkServer& server, const UniValue& params)
{
    ...
    if (g_second_stage.count(server.aux_pow_path)) {
        LogPrint(BCLog::MERGEMINE, "Removing unsolved second-stage work unit for chain 0x%s", HexStr(server.aux_pow_path.begin(), server.aux_pow_path.end()));
...
void MergeMiningManagerThread()
{
    ...
    if (!gArgs.IsArgSet("-mergeminename")) {
        ...
        chain_names["freicoin"] = CreateChainParams(CBaseChainParams::MAIN)->DefaultAuxPowPath();
        chain_names["tradecraft"] = chain_names["freicoin"];
        // Freicoin / Tradecraft test network.
        chain_names["testnet"] = CreateChainParams(CBaseChainParams::TESTNET)->DefaultAuxPowPath();
        // Freicoin / Tradecraft RPC test network.
        chain_names["regtest"] = CreateChainParams(CBaseChainParams::REGTEST)->DefaultAuxPowPath();
    } else {
