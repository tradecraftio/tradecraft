...
static void merge_mining_read_cb(bufferevent *bev, void *ctx)
{
    ...
    if (!g_mergemine_conn.count(bev)) {
        LogPrint("mergemine", "Received read notification for unknown auxiliary work source on connection 0x%x\n", (size_t)bev);
        return;
    }
    ...
    while (!done && (cstr = evbuffer_readln(input, &len, EVBUFFER_EOL_CRLF))) {
        ...
        free(cstr);
        LogPrint("mergemine", "Received line of data from auxiliary work source stratum+tcp://%s (%s) : %s\n", server.socket.ToString(), server.name, line);
        ...
        try {
            if (isreply && val["id"].get_int() == -1) {
                ...
                LogPrint("mergemine", "Setting extranonce1 to \"%s\" and extranonce2_size to %d from stratum+tcp://%s (%s)\n", HexStr(extranonce1.begin(), extranonce1.end()), extranonce2_size, server.socket.ToString(), server.name);
        ...
        LogPrint("stratum", "Sending stratum response to stratum+tcp://%s (%s) : %s", server.socket.ToString(), server.name, reply);
...
static void merge_mining_event_cb(bufferevent *bev, short what, void *ctx)
{
    ...
    // Lookup the client record for this connection
    if (!g_mergemine_conn.count(bev)) {
        LogPrint("mergemine", "Received event notification for unknown auxiliary work source on connection 0x%x\n", (size_t)bev);
        return;
    }
    AuxWorkServer& server = g_mergemine_conn[bev];
...
void MergeMiningManagerThread()
{
    ...
    if (!IsArgSet("-mergeminename")) {
        ...
        const std::vector<std::string>& vnames = mapMultiArgs.at("-mergeminename");
...
void StopMergeMining()
{
    ...
    for (auto& server : g_mergemine_conn) {
        LogPrint("mergemine", "Closing stratum connection to stratum+tcp://%s (%s) due to process termination\n", server.second.socket.ToString(), server.second.name);
