                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (!CheckInputs(tx, state, view, 0, true, scriptVerifyFlags, true, txdata)) {
            // SCRIPT_VERIFY_CLEANSTACK requires SCRIPT_VERIFY_WITNESS, so we
            // need to turn both off, and compare against just turning off CLEANSTACK
            // to see if the failure is specifically due to witness validation.
            if (tx.wit.IsNull() && CheckInputs(tx, state, view, 0, true, scriptVerifyFlags & ~(SCRIPT_VERIFY_WITNESS | SCRIPT_VERIFY_CLEANSTACK), true, txdata) &&
                !CheckInputs(tx, state, view, 0, true, scriptVerifyFlags & ~SCRIPT_VERIFY_CLEANSTACK, true, txdata)) {
        ...
        if (!CheckInputs(tx, state, view, 0, true, MANDATORY_SCRIPT_VERIFY_FLAGS, true, txdata))
        {
...
bool IsTriviallySpendable(const CCoins& from, const COutPoint& prevout, unsigned int flags)
{
    ...
    // Ideally we shouldn't need this structure, since it requires
    // some hash operations to setup and is never used.  However
    // CScriptCheck calls VerifyScript with a signature checker that
    // is constructed with a reference to this struct, so until we
    // refactor, it needs to exist.
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // block.nVersion=3 blocks became manditory:
    ...
    if (enforce_block_final) {
        ...
        if (prev_final_tx->IsCoinBase() && (pindex->nHeight - prev_final_tx->nHeight < COINBASE_MATURITY)) {
            ...
            BOOST_FOREACH(const CTxIn& txin, final_tx.vin) {
                ...
                // The other source of inputs that is allowed is the
                // coinbase transaction whose outputs matured in this
                // block.  We compare with `>=` because the protocol
                // cleanup rule change might shorten coinbase maturity
                // rules for the compatibility chain.
                if (from->IsCoinBase() && (from->nHeight >= (pindex->nHeight - COINBASE_MATURITY))) {
                    continue;
                }
                // Otherwise we must be, by method of exclusion, spending of an
                // already-matured coin which doesn't fit into the above
                // categories.
            ...
            if (spends_prior_tx != prev_final_tx->vout.size()) {
            ...
            // allowed to have as many outputs as has inputs.
            ...
            BOOST_FOREACH(const CTxOut& txout, final_tx.vout) {
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (!CheckInputs(tx, state, view, !truncate_inputs + !use_alu, fScriptChecks, flags, fCacheResults, txdata[i], nScriptCheckThreads ? &vChecks : NULL))
...
void GenerateCoinbaseCommitment(CBlock& block, const CBlockIndex* pindexPrev, const Consensus::Params& consensusParams)
{
    ...
    if (IsWitnessEnabled(pindexPrev, consensusParams)) {
        ...
        script[36] = 0x49;
        script[37] = 0x48;
        swap(script, const_cast<CTxOut*>(&block.vtx.back().vout.back())->scriptPubKey);
        block.vtx.back().UpdateHash();
        uint256 witnessroot = BlockWitnessMerkleRoot(block);
        unsigned char* scriptPubKey = &const_cast<CTxOut*>(&block.vtx.back().vout.back())->scriptPubKey[0];
        scriptPubKey[1] = 0x01;
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, CBlockIndex * const pindexPrev, int64_t nAdjustedTime, bool fCheckPOW)
{
    // Check for activation of rule changes
                     ...
                     ++mi)
                {
                    ...
                    else if (!fMissingInputs2)
                    {
                        ...
                        if (orphanTx.wit.IsNull() && !stateDummy.CorruptionPossible()) {
                            ...
                            // See https://github.com/freicoin/freicoin/issues/8279 for details.
                            assert(recentRejects);
