...
/** Fees smaller than this (in kria) are considered zero fee (for relaying, mining and transaction creation) */
...
bool IsInitialBlockDownload()
{
    ...
    if (lockIBDState)
        return false;
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 * 14 ||
...
bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int per_input_adjustment, int nSpendHeight, bool protocol_cleanup)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if ((coins->vout[prevout.n].GetReferenceValue() || !protocol_cleanup) && (tx.lock_height < coins->refheight))
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (initial_block_final) {
        ...
        for (idx = 0; idx < (uint32_t)coinbaseTx.vout.size(); ++idx) {
            if (!IsTriviallySpendable(coinbaseTx, idx, flags|SCRIPT_VERIFY_CLEANSTACK)) {
    ...
    if (enforce_block_final) {
        ...
        if (prev_final_tx->IsCoinBase() && (pindex->nHeight - prev_final_tx->nHeight < COINBASE_MATURITY)) {
            ...
            BOOST_FOREACH(const CTxOut& txout, final_tx.vout) {
                if (!IsTriviallySpendable(final_tx, 0, flags|SCRIPT_VERIFY_CLEANSTACK)) {
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (!CheckInputs(tx, state, view, !truncate_inputs + !use_alu, fScriptChecks, flags, fCacheResults, nScriptCheckThreads ? &vChecks : NULL))
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Check proof of work
    if (!protocol_cleanup && (block.nBits != GetNextWorkRequired(pindexPrev, &block, consensusParams)))
    ...
    if (!protocol_cleanup && (block.nVersion < 2 && IsSuperMajority(2, pindexPrev, consensusParams.nMajorityRejectBlockOutdated, consensusParams)))
        return state.Invalid(error("%s: rejected nVersion=1 block", __func__),
                             REJECT_OBSOLETE, "bad-version");

    // Reject block.nVersion=2 blocks when 95% (75% on testnet) of the network has upgraded:
    if (!protocol_cleanup && (block.nVersion < 3 && IsSuperMajority(3, pindexPrev, consensusParams.nMajorityRejectBlockOutdated, consensusParams)))
