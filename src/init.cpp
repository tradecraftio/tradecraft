...
{
    return AbsPathForConfigVal(fs::path(args.GetArg("-pid", FREICOIN_PID_FILENAME)));
}
...
void SetupServerArgs(NodeContext& node)
{
    ...
    argsman.AddArg("-incrementalrelayfee=<amt>", strprintf("Fee rate (in %s/kB) used to define cost of relay, used for mempool limiting and transaction replacement. (default: %s)", CURRENCY_UNIT, FormatMoney(DEFAULT_INCREMENTAL_RELAY_FEE)), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::NODE_RELAY);
...
bool AppInitMain(const util::Ref& context, NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    fs::path config_file_path = GetConfigFile(args.GetArg("-conf", FREICOIN_CONF_FILENAME));
