...
// Copyright (c) 2009-2018 The Freicoin developers
...
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", FREICOIN_PID_FILENAME)));
}
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", FREICOIN_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", FREICOIN_PID_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-bitcoinmode", strprintf("Enable bitcoin unit test compatibility mode, where certain Freicoin-specific consensus rules are ignored (-regtest only; default: %u)", DEFAULT_BITCOINMODE), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    ...
    gArgs.AddArg("-incrementalrelayfee=<amt>", strprintf("Fee rate (in %s/kB) used to define cost of relay, used for mempool limiting and transaction replacement. (default: %s)", CURRENCY_UNIT, FormatMoney(DEFAULT_INCREMENTAL_RELAY_FEE)), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::NODE_RELAY);
...
/** Sanity checks
 *  Ensure that Freicoin is running in a usable environment with all
...
bool AppInitParameterInteraction()
{
    ...
    if (gArgs.GetBoolArg("-bitcoinmode", DEFAULT_BITCOINMODE)) {
        ...
        if (!chainparams.MineBlocksOnDemand()) {
            return InitError(_("Bitcoin unit test compatibility mode only allowed on regtest.").translated);
        }
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", FREICOIN_CONF_FILENAME));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                if (::ChainstateActive().CoinsDB().NeedsReindex()) {
                    strLoadError = _("You need to rebuild the coin database and block-undo records using -reindex when upgrading from a pre-v15 chainstate database.  If pruning is enabled, this will redownload the entire blockchain.").translated;
                    break;
                }
