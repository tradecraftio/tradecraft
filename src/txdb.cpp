    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                if (!CheckAuxiliaryProofOfWork(pindexNew->GetBlockHeader(), Params().GetConsensus()) || (!IsProtocolCleanupActive(Params().GetConsensus(), pindexNew) && !CheckProofOfWork(pindexNew->GetBlockHeader(), Params().GetConsensus())))
