...
isminetype IsMine(const CKeyStore &keystore, const CScript& scriptPubKey, bool& isInvalid, SigVersion sigversion)
{
    ...
    case TX_WITNESS_V0_LONGHASH:
    {
        ...
        if (whichType == TX_WITNESS_V0_SHORTHASH) {
            found = keystore.GetWitnessV0Script(WitnessV0ShortHash(vSolutions[0]), innerscript);
        }
        if (whichType == TX_WITNESS_V0_LONGHASH) {
            found = keystore.GetWitnessV0Script(WitnessV0LongHash(vSolutions[0]), innerscript);
        }
        if (found) {
