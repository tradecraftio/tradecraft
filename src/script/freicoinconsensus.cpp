                                    ...
                                    unsigned int nIn, unsigned int flags, freicoinconsensus_error* err)
{
    ...
    try {
        ...
        return VerifyScript(tx.vin[nIn].scriptSig, CScript(scriptPubKey, scriptPubKey + scriptPubKeyLen), &tx.vin[nIn].scriptWitness, flags, TransactionSignatureChecker(&tx, nIn, amount, refheight, txdata), nullptr);
