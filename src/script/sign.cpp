                     ...
                     std::vector<valtype>& ret, txnouttype& whichTypeRet, SigVersion sigversion)
{
    ...
    case TX_WITNESS_V0_LONGHASH:
    {
        ...
        {
        case TX_WITNESS_V0_SHORTHASH:
            found = creator.KeyStore().GetWitnessV0Script(WitnessV0ShortHash(vSolutions[0]), entry);
            break;

        case TX_WITNESS_V0_LONGHASH:
            found = creator.KeyStore().GetWitnessV0Script(WitnessV0LongHash(vSolutions[0]), entry);
            break;
        }
        if (found) {
            if (!entry.m_script.empty() && (entry.m_script[0] == 0x00)) {
                // The second item on the stack (first to be pushed) is the
                // witness script, which is contained in the
                // WitnessV0ScriptEntry passed back to us.
                ret.emplace_back(entry.m_script.begin(), entry.m_script.end());
                // The first item on the stack (last to be pushed) is the Merkle
                // proof.  We construct the proof from the branch and path
                // fields of the WitnessV0ScriptEntry structure.
                ret.emplace_back();
                // The path is specified in zero to four bytes in little endian
                // order.  The exact number of bytes is implicit since the size
                // of the field which follows is known to be a multiple of 32.
                // So we add all the bytes of path, and then remove any ending
                // zero bytes, which are to be understood implicitly.
                ...
                // The branch hashes are serialized in order, without a length
                // specifier or padding bytes.
