                     ...
                     std::vector<valtype>& ret, txnouttype& whichTypeRet, SigVersion sigversion)
{
    ...
    case TX_WITNESS_V0_LONGHASH:
    {
        ...
        {
        case TX_WITNESS_V0_SHORTHASH:
            found = creator.KeyStore().GetWitnessV0Script(WitnessV0ShortHash(vSolutions[0]), witscript);
            break;

        case TX_WITNESS_V0_LONGHASH:
            found = creator.KeyStore().GetWitnessV0Script(WitnessV0LongHash(vSolutions[0]), witscript);
            break;
        }
        if (found) {
