                     ...
                     std::vector<valtype>& ret, txnouttype& whichTypeRet, SigVersion sigversion, SignatureData& sigdata)
{
    ...
    case TX_WITNESS_V0_LONGHASH:
    {
        ...
        if (whichTypeRet == TX_WITNESS_V0_SHORTHASH) {
            sigdata.missing_witness_script = WitnessV0ShortHash(vSolutions[0]);
        } else if (whichTypeRet == TX_WITNESS_V0_LONGHASH) {
            sigdata.missing_witness_script = WitnessV0ShortHash(WitnessV0LongHash(vSolutions[0]));
        }
        return false;
    }
...
bool ProduceSignature(const SigningProvider& provider, const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata)
{
    ...
    if (solved && whichType == TX_SCRIPTHASH)
    {
        ...
        solved = solved && SignStep(provider, creator, subscript, result, whichType, SigVersion::BASE, sigdata) && whichType != TX_SCRIPTHASH && whichType != TX_WITNESS_V0_LONGHASH && whichType != TX_WITNESS_V0_SHORTHASH;
        P2SH = true;
    }
...
bool IsSegWitOutput(const SigningProvider& provider, const CScript& script)
{
    ...
    if (whichtype == TX_WITNESS_V0_LONGHASH || whichtype == TX_WITNESS_V0_SHORTHASH || whichtype == TX_WITNESS_UNKNOWN) return true;
...
bool SignTransaction(CMutableTransaction& mtx, const SigningProvider* keystore, const std::map<COutPoint, Coin>& coins, int nHashType, std::map<int, std::string>& input_errors)
{
    ...
    for (unsigned int i = 0; i < mtx.vin.size(); i++) {
        ...
        if (!fHashSingle || (i < mtx.vout.size())) {
            ProduceSignature(*keystore, MutableTransactionSignatureCreator(&mtx, i, value, refheight, nHashType), prevPubKey, sigdata);
        }
        ...
        if (!VerifyScript(txin.scriptSig, prevPubKey, &txin.scriptWitness, STANDARD_SCRIPT_VERIFY_FLAGS, TransactionSignatureChecker(&txConst, i, value, refheight), &serror)) {
