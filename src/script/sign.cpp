...
MutableTransactionSignatureCreator::MutableTransactionSignatureCreator(const CMutableTransaction* txToIn, unsigned int nInIn, const CAmount& amountIn, int64_t refheightIn, int nHashTypeIn)
    : txTo(txToIn), nIn(nInIn), nHashType(nHashTypeIn), amount(amountIn), refheight(refheightIn), checker(txTo, nIn, amountIn, refheightIn, MissingDataBehavior::FAIL),
      m_txdata(nullptr)
{
}

MutableTransactionSignatureCreator::MutableTransactionSignatureCreator(const CMutableTransaction* txToIn, unsigned int nInIn, const CAmount& amountIn, int64_t refheightIn, const PrecomputedTransactionData* txdata, int nHashTypeIn)
    : txTo(txToIn), nIn(nInIn), nHashType(nHashTypeIn), amount(amountIn), refheight(refheightIn),
...
bool SignTransaction(CMutableTransaction& mtx, const SigningProvider* keystore, const std::map<COutPoint, Coin>& coins, int nHashType, std::map<int, std::string>& input_errors)
{
    ...
    for (unsigned int i = 0; i < mtx.vin.size(); i++) {
        ...
        if (coin == coins.end() || coin->second.IsSpent()) {
            ...
            spent_outputs[i] = {CTxOut(coin->second.out.nValue, coin->second.out.scriptPubKey), coin->second.refheight};
        }
    }
    ...
    for (unsigned int i = 0; i < mtx.vin.size(); i++) {
        ...
        if (!fHashSingle || (i < mtx.vout.size())) {
            ProduceSignature(*keystore, MutableTransactionSignatureCreator(&mtx, i, amount, refheight, &txdata, nHashType), prevPubKey, sigdata);
        }
        ...
        if (!VerifyScript(txin.scriptSig, prevPubKey, &txin.scriptWitness, STANDARD_SCRIPT_VERIFY_FLAGS, TransactionSignatureChecker(&txConst, i, amount, refheight, txdata, MissingDataBehavior::FAIL), &serror)) {
