...
std::string GetWorkUnit(StratumClient& client) EXCLUSIVE_LOCKS_REQUIRED(cs_stratum)
{
    ...
    if (tip != ::ChainActive().Tip() || (mempool.GetTransactionsUpdated() != transactions_updated_last && (GetTime() - last_update_time) > 5) || !work_templates.count(job_id))
    {
        ...
        std::unique_ptr<CBlockTemplate> new_work = BlockAssembler(EnsureMemPool(), Params()).CreateNewBlock(script);
        if (!new_work) {
    ...
    if (client.m_supports_aux && !current_work.GetBlock().m_aux_pow.IsNull() && !current_work.m_aux_hash2 && !client.m_aux_addr.empty()) {
        ...
        for (const auto& addr : client.m_aux_addr) {
            ...
            commits.pushKV(EncodeDestination(addr), HexStr(hash.begin(), hash.end()));
        }
            ...
            if (res) {
                CBlockIndex* block_index = nullptr;
                {
                    ...
                    if (::BlockIndex().count(hash)) {
                        block_index = ::BlockIndex().at(hash);
                    }
                }
                if (!block_index) {
