...
void CustomizeWork(const StratumClient& client, const StratumWork& current_work, const CTxDestination& addr, const std::vector<unsigned char>& extranonce1, const std::vector<unsigned char>& extranonce2, CMutableTransaction& cb, CMutableTransaction& bf, std::vector<uint256>& cb_branch)
{
...
uint256 CustomizeCommitHash(const StratumClient& client, const CTxDestination& addr, const uint256& job_id, const StratumWork& current_work, const uint256& secret)
{
...
std::string GetWorkUnit(StratumClient& client) EXCLUSIVE_LOCKS_REQUIRED(cs_stratum)
{
    ...
    if (!Params().MineBlocksOnDemand() && g_context->connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0) {
    ...
    if (tip != ::ChainActive().Tip() || (mempool.GetTransactionsUpdated() != transactions_updated_last && (GetTime() - last_update_time) > 5) || !work_templates.count(job_id))
    {
        ...
        std::unique_ptr<CBlockTemplate> new_work;
        new_work = BlockAssembler(mempool, Params()).CreateNewBlock(script);
        if (!new_work) {
...
bool SubmitBlock(StratumClient& client, const uint256& job_id, const StratumWork& current_work, const std::vector<unsigned char>& extranonce1, const std::vector<unsigned char>& extranonce2, boost::optional<uint32_t> nVersion, uint32_t nTime, uint32_t nNonce) EXCLUSIVE_LOCKS_REQUIRED(cs_stratum)
{
...
bool SubmitAuxiliaryBlock(StratumClient& client, const CTxDestination& addr, const uint256& job_id, const StratumWork& current_work, CBlockHeader& blkhdr) EXCLUSIVE_LOCKS_REQUIRED(cs_stratum)
{
    CMutableTransaction cb, bf;
    std::vector<uint256> cb_branch;
    static const std::vector<unsigned char> dummy(4, 0x00); // extranonce2
...
static bool StratumBindAddresses(event_base* base, NodeContext& node) EXCLUSIVE_LOCKS_REQUIRED(cs_stratum)
{
    const ArgsManager& args = *Assert(node.args);
    int defaultPort = args.GetArg("-stratumport", BaseParams().StratumPort());
...
bool InitStratumServer(NodeContext& node)
{
    LOCK(cs_stratum);
