...
struct StratumWork {
    ...
    StratumWork(const ChainstateManager& chainman, const CBlockIndex* prev_block_index, const node::CBlockTemplate& block_template);
...
StratumWork::StratumWork(const ChainstateManager& chainman, const CBlockIndex* prev_block_index, const node::CBlockTemplate& block_template)
...
std::string GetWorkUnit(StratumClient& client) EXCLUSIVE_LOCKS_REQUIRED(cs_stratum)
{
    ...
    if (tip != g_context->chainman->ActiveChain().Tip() || (mempool.GetTransactionsUpdated() != transactions_updated_last && (GetTime() - last_update_time) > 5) || !work_templates.count(job_id))
    {
        ...
        work_templates[job_id] = StratumWork(*g_context->chainman, tip_new, *new_work);
        tip = tip_new;
