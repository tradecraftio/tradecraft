...
CAmount CWallet::GetImmatureBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        {
            const CWalletTx* pcoin = &(*it).second;
            nTotal += GetTimeAdjustedValue(pcoin->GetImmatureCredit(), height - (int)pcoin->lock_height);
        }
    }
    return nTotal;
}
...
CAmount CWallet::GetWatchOnlyBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            const CWalletTx* pcoin = &(*it).second;
            if (pcoin->IsTrusted())
                nTotal += GetTimeAdjustedValue(pcoin->GetAvailableWatchOnlyCredit(), height - (int)pcoin->lock_height);
        }
    }
...
CAmount CWallet::GetImmatureWatchOnlyBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        {
            const CWalletTx* pcoin = &(*it).second;
            nTotal += GetTimeAdjustedValue(pcoin->GetImmatureWatchOnlyCredit(), height - (int)pcoin->lock_height);
        }
    }
    return nTotal;
}
 ...
 */
void CWallet::AvailableCoins(vector<COutput>& vCoins, int32_t height, bool fOnlyConfirmed, const CCoinControl *coinControl) const
{
    vCoins.clear();
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, int32_t height, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl) const
{
    ...
    AvailableCoins(vCoins, height, true, coinControl);
