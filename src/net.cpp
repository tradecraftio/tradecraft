...
bool CNode::ReceiveMsgBytes(Span<const uint8_t> msg_bytes, bool& complete)
{
    ...
    m_deserializer->SetMaxMessageLength(MaxProtocolMessageLength(Params().GetConsensus(), max_untrusted_peers, m_last_recv.load().count()));
...
CNode::CNode(NodeId idIn, int max_untrusted_peersIn, ServiceFlags nLocalServicesIn, std::shared_ptr<Sock> sock, const CAddress& addrIn, uint64_t nKeyedNetGroupIn, uint64_t nLocalHostNonceIn, const CAddress& addrBindIn, const std::string& addrNameIn, ConnectionType conn_type_in, bool inbound_onion)
    : m_sock{sock},
