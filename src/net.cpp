...
CNode* CConnman::ConnectNode(CAddress addrConnect, const char *pszDest, bool fCountFailure, ConnectionType conn_type)
{
    ...
    CNode* pnode = new CNode(id, MaxUntrustedPeers(), nLocalServices, GetBestHeight(), hSocket, addrConnect, CalculateKeyedNetGroup(addrConnect), nonce, addr_bind, pszDest ? pszDest : "", conn_type);
    pnode->AddRef();
...
void CNode::copyStats(CNodeStats &stats, const std::vector<bool> &m_asmap)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Freicoin users should be well used to small numbers with many decimal places by now :)
...
static std::size_t MaxProtocolMessageLength(const Consensus::Params &params, size_t max_untrusted_peers, int64_t time)
{
    ...
    if (IsSizeExpansionActive(Params().GetConsensus(), time)) {
...
void CConnman::AcceptConnection(const ListenSocket& hListenSocket) {
    ...
    CNode* pnode = new CNode(id, MaxUntrustedPeers(), nodeServices, GetBestHeight(), hSocket, addr, CalculateKeyedNetGroup(addr), nonce, addr_bind, "", ConnectionType::INBOUND, inbound_onion);
    pnode->AddRef();
...
CNode::CNode(NodeId idIn, int max_untrusted_peersIn, ServiceFlags nLocalServicesIn, int nMyStartingHeightIn, SOCKET hSocketIn, const CAddress& addrIn, uint64_t nKeyedNetGroupIn, uint64_t nLocalHostNonceIn, const CAddress& addrBindIn, const std::string& addrNameIn, ConnectionType conn_type_in, bool inbound_onion)
