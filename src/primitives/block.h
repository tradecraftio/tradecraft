...
#include <node/protocol_version.h> // for AUX_POW_VERSION
...
struct BlockHeaderSerParams {
    const bool allow_auxpow;
    SER_PARAMS_OPFUNC

    template<typename T>
    static ParamsWrapper<BlockHeaderSerParams, T> ForPeerVersion(int ver, T& obj) {
        if (ver < AUX_POW_VERSION) {
            return BlockHeaderSerParams{.allow_auxpow = false}(obj);
        } else {
            return BlockHeaderSerParams{.allow_auxpow = true}(obj);
        }
    }
};
static constexpr BlockHeaderSerParams BLKHDR_WITH_AUXPOW{.allow_auxpow = true};
static constexpr BlockHeaderSerParams BLKHDR_NO_AUXPOW{.allow_auxpow = false};

struct BlockSerParams {
    const bool allow_auxpow;
    const bool allow_witness;
    SER_PARAMS_OPFUNC

    template<typename T>
    static ParamsWrapper<BlockSerParams, T> ForPeerVersion(int ver, TransactionSerParams tx_params, T& obj) {
        if (ver < AUX_POW_VERSION) {
            return BlockSerParams{.allow_auxpow = false, .allow_witness = tx_params.allow_witness}(obj);
        } else {
            return BlockSerParams{.allow_auxpow = true, .allow_witness = tx_params.allow_witness}(obj);
        }
    }
};
static constexpr BlockSerParams BLK_WITH_AUXPOW_AND_WITNESS{.allow_auxpow = true, .allow_witness = true};
static constexpr BlockSerParams BLK_WITH_AUXPOW_NOT_WITNESS{.allow_auxpow = true, .allow_witness = false};
static constexpr BlockSerParams BLK_WITH_WITNESS_NOT_AUXPOW{.allow_auxpow = false, .allow_witness = true};
static constexpr BlockSerParams BLK_NO_AUXPOW_NOR_WITNESS{.allow_auxpow = false, .allow_witness = false};
...
public:
    ...
    template <typename Stream>
    void Serialize(Stream& s) const {
        const bool fAllowAuxPow = s.GetParams().allow_auxpow;
        ...
        unsigned char flags = 0;
        if (fAllowAuxPow) {
            // Check whether auxiliary proof-of-work needs to be serialized.
            if (!m_aux_pow.IsNull()) {
                flags |= 1;
            }
        }
...
public:
    ...
    template <typename Stream>
    void Serialize(Stream& s) const {
        ::Serialize(s, AsBase<CBlockHeader>(*this));
        if (s.GetParams().allow_witness) {
            ParamsStream{TX_WITH_WITNESS, s} << vtx;
        } else {
            ParamsStream{TX_NO_WITNESS, s} << vtx;
        }
    }

    template <typename Stream>
    void Unserialize(Stream& s) {
        ::Unserialize(s, AsBase<CBlockHeader>(*this));
        if (s.GetParams().allow_witness) {
            ParamsStream{TX_WITH_WITNESS, s} >> vtx;
        } else {
            ParamsStream{TX_NO_WITNESS, s} >> vtx;
...
struct CBlockLocator
{
     ...
     **/
    static constexpr int DUMMY_VERSION = 70017;
