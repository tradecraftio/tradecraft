...
std::pair<uint256, uint256> CBlockHeader::GetAuxiliaryHash(const Consensus::Params& params, bool* mutated) const
{
    ...
    CDataStream aux_block_header(SER_GETHASH, PROTOCOL_VERSION | SERIALIZE_BLOCK_NO_AUX_POW);
...
{
    return SerializeHash(*this, SER_GETHASH, PROTOCOL_VERSION | SERIALIZE_BLOCK_NO_AUX_POW);
}
...
int64_t GetBlockWeight(const CBlock& block)
{
    ...
    return ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS | SERIALIZE_BLOCK_NO_AUX_POW) * (WITNESS_SCALE_FACTOR - 1) + ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_BLOCK_NO_AUX_POW);
}
