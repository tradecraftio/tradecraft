...
The RPM spec file provided here is for Freicoin 0.12.0 and builds on CentOS
...
reference the release tarball available from http://freico.in/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Freicoin git repository](https://github.com/tradecraftio/tradecraft) but are not part of
...
Currently only `Source10` is used, for the example `freicoin.conf` file.
...
in the freicoin git repository. At present only an SVG version of the Freicoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the freicoin
...
needed when 0.12.1 is released, a proper fix is already in the Freicoin git
master branch.
...
idea to build Freicoin against a static version of that library compiled
...
The `freicoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `freicoin-server` package. At this point, most RPM based
...
The `freicoin-server` package should have SELinux support. How to properly do
...
Freicoin against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/freicoin/). He has been
packaging Freicoin for Fedora at least since 2012.
...
4. I renamed the `freicoin` package that contains the Qt GUI to `freicoin` as
that appears to be how the general population refers to it, in contrast to
`freicoin-xt` or `freicoin-classic`. I wanted to make sure the general population
