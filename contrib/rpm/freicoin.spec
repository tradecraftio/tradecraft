...
%check
make check
srcdir=src test/freicoin-util-test.py
...
%{_sbindir}/semanage port -a -t freicoin_port_t -p tcp 28638
%{_sbindir}/semanage port -a -t freicoin_port_t -p tcp 28639
...
if [ $1 -eq 0 ]; then
	...
	%{_sbindir}/semanage port -d -p tcp 28638
	%{_sbindir}/semanage port -d -p tcp 28639
	for selinuxvariant in %{selinux_variants}; do
