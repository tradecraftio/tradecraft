...
Tracing (USDT) support for Freicoin on Linux using. For more information on
USDT support in Freicoin see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `freicoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `freicoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Freicoin USDT documentation] on how to list available tracepoints in your
`freicoind` binary.

[Freicoin USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
Freicoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
In a different terminal, starting Freicoin in SigNet mode and with
re-indexing enabled.
