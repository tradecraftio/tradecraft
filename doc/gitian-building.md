...
*Setup instructions for a Gitian build of Freicoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Freicoin
...
- [Building Freicoin](#building-freicoin)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-freicoin-host/README.md).
...
Clone the git repositories for freicoin and Gitian.
...
in the freicoin repository under 'Fetch and build inputs' to install sources which require
...
To build Freicoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the freicoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/freicoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/freicoin/freicoin
...
For example:
```bash
URL=https://github.com/laanwj/freicoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit freicoin=${COMMIT} --url freicoin=${URL} ../freicoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit freicoin=${COMMIT} --url freicoin=${URL} ../freicoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit freicoin=${COMMIT} --url freicoin=${URL} ../freicoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the freicoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../freicoin/contrib/gitian-descriptors/*|sort|uniq )
...
FRCPATH=/some/root/path/freicoin.git
SIGPATH=/some/root/path/freicoin-detached-sigs.git

./bin/gbuild --url freicoin=${FRCPATH},signature=${SIGPATH} ../freicoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/freicoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/freicoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/freicoin-osx-build.assert
```
...
[freicoin/gitian.sigs](https://github.com/freicoin/gitian.sigs/) repository, or if that's not possible create a pull
