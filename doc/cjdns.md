# CJDNS support in Freicoin

It is possible to run Freicoin over CJDNS, an encrypted IPv6 network that
...
done outside of Freicoin, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Freicoin network and individual nodes.
...
before it will work with your Freicoin node. This is described in steps
...
## Run Freicoin with CJDNS

Once you are connected to the CJDNS network, the following Freicoin
...
When enabled, this option tells Freicoin that it is running in an
...
IPv6 local network. This helps Freicoin perform better address management:
...
CJDNS support was added to Freicoin in version v23 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `freicoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/tradecraftio/tradecraft/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Freicoin

There are several ways to see your CJDNS address in Freicoin:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `freicoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `freicoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `freicoin-cli help getnodeaddresses` for details.
