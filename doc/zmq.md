...
The Freicoin daemon can be configured to act as a trusted "border
router", implementing the freicoin wire protocol and relay, making
...
The ZeroMQ feature in Freicoin requires the ZeroMQ API >= 4.0.0
...
For instance:
    ...
    $ freicoind -zmqpubhashtx=tcp://127.0.0.1:28102 \
               -zmqpubhashtx=tcp://192.168.1.2:28102 \
               -zmqpubhashblock="tcp://[::1]:28103" \
...
terminator). These options can also be provided in freicoin.conf.
...
From the perspective of freicoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into freicoind directly. Furthermore, no information is
...
using. Freicoind appends an up-counting sequence number to each
