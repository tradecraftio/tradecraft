...
However if this is done, once the release has been tagged in the freicoin-detached-sigs repo, the `guix-codesign` step must be performed again in order for the guix attestation to be valid when compared against the attestations of non-codesigner builds. The directories created by `guix-codesign` will need to be cleared prior to running `guix-codesign` again.
...
- Upload to the freico.in server:
    1. The contents of each `./freicoin/guix-build-${VERSION}/output/${HOST}/` directory.
...
- After uploading release candidate binaries, notify the freicoin-dev mailing list and
  freicoin-dev group that a release candidate is available for testing. Include a link to the release
  notes draft.
...
- Update the freico.in website

  - blog post

  - maintained versions [table](https://github.com/freicoin/freico.in/commits/master/_includes/posts/maintenance-table.md)
