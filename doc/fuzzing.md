# Fuzzing Freicoin using libFuzzer
...
## Overview of Freicoin fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/freicoin_report.pdf) includes an external overview of the status of Freicoin fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.

## Fuzzing harnesses and output

[`process_message`](https://github.com/tradecraftio/tradecraft/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/tradecraftio/tradecraft/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/tradecraftio/tradecraft/tree/master/src/test/fuzz).
...
It is possible to specify `freicoind` arguments to the `fuzz` executable.
...
The project's collection of seed corpora is found in the [`freicoin/qa-assets`](https://github.com/freicoin/qa-assets) repo.

To fuzz `process_message` using the [`freicoin/qa-assets`](https://github.com/freicoin/qa-assets) seed corpus:

```sh
$ git clone https://github.com/freicoin/qa-assets
...
Fuzzing on a harness compiled with `--with-sanitizers=address,fuzzer,undefined` is good for finding bugs. However, the very slow execution even under libFuzzer will limit the ability to find new coverage. A good approach is to perform occasional long runs without the additional bug-detectors (configure `--with-sanitizers=fuzzer`) and then merge new inputs into a corpus as described in the qa-assets repo (https://github.com/freicoin/qa-assets/blob/main/.github/PULL_REQUEST_TEMPLATE.md).  Patience is useful; even with improved throughput, libFuzzer may need days and 10s of millions of executions to reach deep/hard targets.
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`freicoin/qa-assets`](https://github.com/freicoin/qa-assets) repo.

Every single pull request submitted against the Freicoin repo is automatically tested against all inputs in the [`freicoin/qa-assets`](https://github.com/freicoin/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Freicoin more robust.
...
with certain assembly code from Freicoin's code. See [developer notes on sanitizers](https://github.com/tradecraftio/tradecraft/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing Freicoin using afl++

## Quickstart guide

To quickly get started fuzzing Freicoin using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Freicoin using Honggfuzz
...
$ git apply << "EOF"
diff --git a/src/freicoind.cpp b/src/freicoind.cpp
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_freicoin/latest).
