...
class WalletTaprootTest(FreicoinTestFramework):
    ...
    def do_test_pst(self, comment, pattern, privmap, treefn, keys_pay, keys_change):
        self.log.info("Testing %s through PST" % comment)

        # Create wallets
        wallet_uuid = uuid.uuid4().hex
        self.nodes[0].createwallet(wallet_name=f"pst_online_{wallet_uuid}", descriptors=True, disable_private_keys=True, blank=True)
        self.nodes[1].createwallet(wallet_name=f"pst_offline_{wallet_uuid}", descriptors=True, blank=True)
        self.nodes[1].createwallet(f"key_only_wallet_{wallet_uuid}", descriptors=True, blank=True)
        pst_online = self.nodes[0].get_wallet_rpc(f"pst_online_{wallet_uuid}")
        pst_offline = self.nodes[1].get_wallet_rpc(f"pst_offline_{wallet_uuid}")
        ...
        assert_equal(self.nodes[0].getdescriptorinfo(desc_pay)['descriptor'], desc_pay_pub)
        assert_equal(self.nodes[0].getdescriptorinfo(desc_change)['descriptor'], desc_change_pub)
        result = pst_online.importdescriptors([{"desc": desc_pay_pub, "active": True, "timestamp": "now"}])
        assert result[0]['success']
        result = pst_online.importdescriptors([{"desc": desc_change_pub, "active": True, "timestamp": "now", "internal": True}])
        assert result[0]['success']
        result = pst_offline.importdescriptors([{"desc": desc_pay, "active": True, "timestamp": "now"}])
        assert result[0]['success']
        ...
        address_type = "bech32m" if "tr" in pattern else "bech32"
        for i in range(4):
            addr_g = pst_online.getnewaddress(address_type=address_type)
            if treefn is not None:
                addr_r = self.make_addr(treefn, keys_pay, i)
                assert_equal(addr_g, addr_r)
            boring_balance = int(self.boring.getbalance() * 100000000)
            ...
            for wallet in [pst_offline, key_only_wallet]:
                ...
                decoded = wallet.decodepst(res["pst"])
                if pattern.startswith("tr("):
                    for pstin in decoded["inputs"]:
                        ...
                        if "taproot_script_path_sigs" in pstin:
                            assert "taproot_merkle_root" in pstin
                            assert "taproot_scripts" in pstin
            ...
            self.generatetoaddress(self.nodes[0], 1, self.boring.getnewaddress(), sync_fun=self.no_op)
            assert pst_online.gettransaction(txid)['confirmations'] > 0

        # Cleanup
        pst = pst_online.sendall(recipients=[self.boring.getnewaddress()], options={"pst": True})["pst"]
        res = pst_offline.walletprocesspst(pst=pst, finalize=False)
        rawtx = self.nodes[0].finalizepst(res['pst'])['hex']
        txid = self.nodes[0].sendrawtransaction(rawtx)
        self.generatetoaddress(self.nodes[0], 1, self.boring.getnewaddress(), sync_fun=self.no_op)
        assert pst_online.gettransaction(txid)['confirmations'] > 0
        pst_online.unloadwallet()
        pst_offline.unloadwallet()
    ...
    def do_test(self, comment, pattern, privmap, treefn):
        ...
        self.do_test_pst(comment, pattern, privmap, treefn, keys[2*nkeys:3*nkeys], keys[3*nkeys:4*nkeys])
