...
class WalletGroupTest(FreicoinTestFramework):
    ...
    def run_test(self):
        ...
        self.nodes[0].generate(1)
        self.sync_all()
        with self.nodes[3].assert_debug_log(['Fee non-grouped = 2920, grouped = 4280, using grouped']):
        ...
        self.nodes[0].generate(1)
        self.sync_all()
        with self.nodes[3].assert_debug_log(['Fee non-grouped = 5660, grouped = 8420, using non-grouped']):
        ...
        self.nodes[0].generate(1)
        self.sync_all()
        with self.nodes[4].assert_debug_log(['Fee non-grouped = 5660, grouped = 8420, using grouped']):
