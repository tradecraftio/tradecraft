...
class BlockFinalTxTest(ComparisonTestFramework):
    ...
    def get_tests(self):
        ...
        assert_equal(self.get_bip9_status(bipName)['status'], 'defined')
        assert_equal(self.get_bip9_status(bipName)['since'], 0)
        tmpl = self.nodes[0].getblocktemplate({'rules':[bipName]})
        ...
        spend_tx.vout.append(CTxOut(int(utxo['value']*100000000) - 10000, CScript([OP_TRUE])))
        spend_tx.nLockTime = 0
        spend_tx.rehash()
        ...
        spend_tx.vout.append(CTxOut(int(utxo['value']*100000000), CScript([OP_TRUE])))
        spend_tx.nLockTime = 0
        spend_tx.rehash()
        try:
            self.nodes[0].sendrawtransaction(ToHex(spend_tx))
