...
class SegWitTest(FreicoinTestFramework):
    ...
    def build_next_block(self, nVersion=4):
        ...
        try:
            finaltx_prevout = self.nodes[0].getblocktemplate({'rules':['finaltx','segwit']})['finaltx']['prevout']
        except KeyError:
            finaltx_prevout = []
        if finaltx_prevout:
    ...
    def test_standardness_v0(self, segwit_activated):
        ...
        if segwit_activated:
            ...
            tx3.vout = [CTxOut(p2sh_tx.vout[0].nValue-1000, CScript([OP_TRUE]))]
            tx3.rehash()
            block = self.build_next_block()
    ...
    def test_getblocktemplate_before_lockin(self):
        ...
        for node in [self.nodes[0], self.nodes[2]]:
            gbt_results = node.getblocktemplate({"rules" : ["finaltx"]})
    ...
    def run_test(self):
        ...
        # Get the block chain going so mining services turn on
        self.nodes[0].generate(1)
