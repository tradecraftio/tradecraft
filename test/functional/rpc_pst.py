    ...
    def run_test(self):
        ...
        for out in decoded['vout']:
            if out['scriptPubKey']['addresses'][0] == p2sh:
                ...
                p2wsh_pos = out['n']
            elif out['scriptPubKey']['addresses'][0] == p2wpk:
                p2wpk_pos = out['n']
            elif out['scriptPubKey']['addresses'][0] == p2sh_p2wsh:
                p2sh_p2wsh_pos = out['n']
            elif out['scriptPubKey']['addresses'][0] == p2sh_p2wpk:
        ...
        res3 = self.nodes[1].walletcreatefundedpst(inputs, outputs, 0, refheight, {"fee_rate": "0.99999999", "add_inputs": True})
        ...
        for param in ["fee_rate", "feeRate"]:
            assert_equal(self.nodes[1].walletcreatefundedpst(inputs, outputs, 0, refheight, {param: 0, "add_inputs": True})["fee"], 0)
        ...
        for param, value in {("fee_rate", 100000), ("feeRate", 1)}:
            ...
            assert_raises_rpc_error(-3, "Invalid amount",
                self.nodes[1].walletcreatefundedpst, inputs, outputs, 0, refheight, {param: "", "add_inputs": True})
        ...
        pstx_info = self.nodes[0].walletcreatefundedpst([{"txid":unspent["txid"], "vout":unspent["vout"]}], [{self.nodes[2].getnewaddress():unspent["value"]+1}], block_height+2, unspent["refheight"], {"add_inputs": True}, False)
        ...
        pstx_info = self.nodes[0].walletcreatefundedpst([{"txid":unspent["txid"], "vout":unspent["vout"]}], [{self.nodes[2].getnewaddress():unspent["value"]+1}], block_height, unspent["refheight"], {"add_inputs": True}, True)
        decoded_pst = self.nodes[0].decodepst(pstx_info["pst"])
        for tx_in, pst_in in zip(decoded_pst["tx"]["vin"], decoded_pst["inputs"]):
            assert_equal(tx_in["sequence"], MAX_SEQUENCE_NUMBER)
