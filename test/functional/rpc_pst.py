...
class PSTTest(FreicoinTestFramework):
    ...
    def test_pst_incomplete_after_invalid_modification(self):
        self.log.info("Check that PST is correctly marked as incomplete after invalid modification")
        node = self.nodes[2]
        ...
        pst = wallet.createpst([{"txid": utxos[0]["txid"], "vout": utxos[0]["vout"]}], [{wallet.getnewaddress(): 0.9999}])
        signed_pst = wallet.walletprocesspst(pst)["pst"]

        # Modify the raw transaction by changing the output address, so the signature is no longer valid
        signed_pst_obj = PST.fromhex(signed_pst)
        ...
        signed_pst_obj.g.map[PST_GLOBAL_UNSIGNED_TX] = bytes.fromhex(raw)

        # Check that the walletprocesspst call succeeds but also recognizes that the transaction is not complete
        signed_pst_incomplete = wallet.walletprocesspst(signed_pst_obj.hex(), finalize=False)
        assert signed_pst_incomplete["complete"] is False
    ...
    def test_input_confs_control(self):
        ...
        pstx1 = wallet.walletcreatefundedpst([], {target_address: 0.1}, 0, 0, {'fee_rate': 1, 'maxconf': 0})['pst']
        ...
        assert_raises_rpc_error(-4, "The preselected coins total amount does not cover the transaction target. Please allow other inputs to be automatically selected or include more coins manually", wallet.walletcreatefundedpst, [{'txid': utxo1['txid'], 'vout': utxo1['vout']}], {target_address: 1}, 0, 0, {'add_inputs': False})

        self.log.info("Fail to craft a new PST with minconf above highest one")
        assert_raises_rpc_error(-4, "Insufficient funds", wallet.walletcreatefundedpst, [{'txid': utxo1['txid'], 'vout': utxo1['vout']}], {target_address: 1}, 0, 0, {'add_inputs': True, 'minconf': 3, 'fee_rate': 10})
        ...
        pst_invalid = wallet.walletcreatefundedpst([{'txid': utxo1['txid'], 'vout': utxo1['vout']}], {target_address: 1}, 0, 0, {'add_inputs': True, 'maxconf': 0, 'fee_rate': 10})['pst']
        ...
        pstx2 = wallet.walletcreatefundedpst([{'txid': utxo1['txid'], 'vout': utxo1['vout']}], {target_address: 1}, 0, 0, {'add_inputs': True, 'minconf': 2, 'fee_rate': 10})['pst']
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(-4, f"Maximum transaction weight must be between {min_tx_weight} and {MAX_STANDARD_TX_WEIGHT}", self.nodes[0].walletcreatefundedpst, [], dest_arg, 0, 0, {"max_tx_weight": -1})
        assert_raises_rpc_error(-4, f"Maximum transaction weight must be between {min_tx_weight} and {MAX_STANDARD_TX_WEIGHT}", self.nodes[0].walletcreatefundedpst, [], dest_arg, 0, 0, {"max_tx_weight": 0})
        assert_raises_rpc_error(-4, f"Maximum transaction weight must be between {min_tx_weight} and {MAX_STANDARD_TX_WEIGHT}", self.nodes[0].walletcreatefundedpst, [], dest_arg, 0, 0, {"max_tx_weight": MAX_STANDARD_TX_WEIGHT + 1})
        ...
        assert_raises_rpc_error(-4, "Maximum transaction weight is less than transaction weight without inputs", self.nodes[0].walletcreatefundedpst, [], dest_arg_large, 0, 0, {"max_tx_weight": min_tx_weight})
        assert_raises_rpc_error(-4, "Maximum transaction weight is less than transaction weight without inputs", self.nodes[0].walletcreatefundedpst, [], dest_arg_large, 0, 0, {"max_tx_weight": large_tx_weight_without_inputs})
        ...
        pst = self.nodes[0].walletcreatefundedpst(outputs=dest_arg,locktime=0, options={"max_tx_weight": max_tx_weight_sufficient})["pst"]
        weight = self.nodes[0].decodepst(pst)["tx"]["weight"]
        ...
        pstx = wmulti.walletcreatefundedpst(inputs=[{"txid":txid,"vout":p2wsh_pos},{"txid":txid,"vout":p2sh_pos},{"txid":txid,"vout":p2sh_p2wsh_pos}], outputs={self.nodes[1].getnewaddress():29.99}, lockheight=refheight, changeAddress=self.nodes[1].getrawchangeaddress())['pst']
        ...
        pst_orig = self.nodes[0].createpst([utxo1, utxo2], {self.nodes[0].getnewaddress():25.999})
        ...
        pst = self.nodes[1].createpst([utxo1, utxo2, utxo3], {self.nodes[0].getnewaddress():32.999})
        ...
        self.generate(self.nodes[0], 6)
        pst2 = self.nodes[1].createpst([utxo4], {self.nodes[0].getnewaddress():Decimal('4.999')})
        ...
        self.generate(self.nodes[0], 6)[0]
        pst = self.nodes[1].createpst([utxo], {self.nodes[0].getnewaddress("", "p2sh-segwit"):Decimal('6.999')})
        ...
        analysis = self.nodes[0].analyzepst('')
        assert_equal(analysis['next'], 'creator')
        ...
        analysis = self.nodes[0].analyzepst('')
        assert_equal(analysis['next'], 'creator')
        assert_equal(analysis['error'], 'PST is not valid. Output amount invalid')

        assert_raises_rpc_error(-22, "TX decode failed", self.nodes[0].analyzepst, "")

        assert_raises_rpc_error(-22, "TX decode failed", self.nodes[0].walletprocesspst, "")
        ...
        scriptsig_hex = pst_in["final_scriptSig"]["hex"] if "final_scriptSig" in pst_in else ""
        witness_stack_hex = pst_in["final_scriptwitness"] if "final_scriptwitness" in pst_in else None
        ...
        if self.options.descriptors:
            ...
            pst = self.nodes[0].createpst([utxo], [{self.nodes[0].getnewaddress(): 0.9999}])
        ...
        pst = PST()
        pst.g = PSTMap({PST_GLOBAL_UNSIGNED_TX: tx.serialize()})
        pst.i = [PSTMap({bytes([PST_IN_WITNESS_UTXO]) : acs_prevout.serialize() + b'\x01\x00\x00\x00'})]
        pst.o = [PSTMap()]
        ...
        pst = self.nodes[2].createpst([utxo], {self.nodes[0].getnewaddress(): 0.99999})
