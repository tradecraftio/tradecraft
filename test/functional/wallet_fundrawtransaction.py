...
class RawTransactionsTest(FreicoinTestFramework):
    ...
    def test_spend_2of2(self):
        ...
        inputs = []
        outputs = {self.nodes[1].getnewaddress():1.1}
        funded_pst = wmulti.walletcreatefundedpst(inputs=inputs, outputs=outputs, options={'changeAddress': w2.getrawchangeaddress()})['pst']

        signed_pst = w2.walletprocesspst(funded_pst)
        final_pst = w2.finalizepst(signed_pst['pst'])
        self.nodes[2].sendrawtransaction(final_pst['hex'])
        self.generate(self.nodes[2], 1)
    ...
    def test_add_inputs_default_value(self):
        ...
        # Case (3), Explicit add_inputs=true and preset inputs (with preset inputs not-covering the target amount)
        options["add_inputs"] = True
        assert "pst" in wallet.walletcreatefundedpst(outputs=[{addr1: 8}], inputs=inputs, options=options)
        ...
        pst_tx = wallet.walletcreatefundedpst(outputs=[{addr1: 8}], inputs=inputs, options=options)
        # Check that only the preset inputs were added to the tx
        ...
        assert "pst" in wallet.walletcreatefundedpst(inputs=[], outputs=outputs, options=options)

        # Case (6). Explicit add_inputs=false, no preset inputs:
        options = {"add_inputs": False}
        assert_raises_rpc_error(-4, ERR_NOT_ENOUGH_PRESET_INPUTS, wallet.walletcreatefundedpst, inputs=[], outputs=outputs, options=options)
