...
class ConfArgsTest(FreicoinTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        with open(self.nodes[0].datadir_path / 'freicoin.conf', 'a', encoding='utf-8') as conf:
        ...
        main_conf_file_path = self.nodes[0].datadir_path / "freicoin_main.conf"
    ...
    def test_config_file_log(self):
        ...
        # directory by freicoind.
        env, default_datadir = util.get_temp_default_datadir(Path(self.options.tmpdir, "test_config_file_log"))
        default_datadir.mkdir(parents=True)

        # Write a freicoin.conf file in the default data directory containing a
        # datadir= line pointing at the node datadir.
        node = self.nodes[0]
        conf_text = node.freicoinconf.read_text()
        conf_path = default_datadir / "freicoin.conf"
        ...
        # (conf_path, not node.freicoinconf)
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a freicoin.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "freicoin.conf" file which is ignored, because a '
        ...
        # the same freicoin.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/freicoin.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when freicoin.conf in the default data directory sets another datadir '
                      'and it contains a different freicoin.conf file that would be ignored')
        ...
        # freicoin.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "freicoin.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "freicoin.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/freicoin.conf" from data directory "{default_datadir}" '
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "freicoin.conf"
        ...
        none_existent_conf_file = default_data_dir / "none_existent_freicoin.conf"
