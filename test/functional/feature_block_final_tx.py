...
class BlockFinalTxTest(FreicoinTestFramework):
    ...
    def get_bip9_status(self, key):
        ...
        if key in info['softforks']:
            return info['softforks'][key]['bip9']
        else:
            raise KeyError(repr(info['softforks']))
            return info['softforks']['!' + key]['bip9']
    ...
    def run_test(self):
        ...
        assert_equal(self.get_bip9_status('finaltx')['status'], 'defined')
        assert_equal(self.get_bip9_status('finaltx')['since'], 0)
        tmpl = node.getblocktemplate({'rules':['segwit','finaltx','auxpow']})
        ...
        assert_equal(self.get_bip9_status('finaltx')['statistics']['elapsed'], 0)
