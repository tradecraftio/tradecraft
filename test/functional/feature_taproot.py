...
def make_spender(comment, *, tap=None, witv0=False, script=None, pk=None, p2sh=False, spk_mutate_pre_p2sh=None, failure=None, standard=True, err_msg=None, sigops_weight=0, need_vin_vout_mismatch=False, **kwargs):
    ...
    * sigops_weight: the pre-taproot sigops weight consumed by a successful spend
    ...
    if witv0:
        ...
        if pk is not None:
            # P2WPK
            script = CScript([pk, OP_CHECKSIG])
            spk = CScript([OP_0, ripemd160(hash256(b'\x00' + script))])
...
def spenders_taproot_active():
    ...
    # Test that MAX_SCRIPT_ELEMENT_SIZE is not consensus-enforced on tapscript (but 80 bytes is standard while 81 is not).
    ...
    add_spender(spenders, "tapscript/inputmaxlimit", leaf="t0", **common, standard=False, inputs=[getter("sign"), random_bytes(MAX_SCRIPT_ELEMENT_SIZE)])
    add_spender(spenders, "tapscript/inputmaxlimit", leaf="t0", **common, standard=False, inputs=[getter("sign"), random_bytes(MAX_SCRIPT_ELEMENT_SIZE+1)])
        ...
        for p2sh in [False, True]:
            for witv0 in [False, True]:
                for hashtype in VALID_SIGHASHES_ECDSA + [random.randrange(0x04, 0x80), random.randrange(0x84, 0x100)]:
                    ...
                    add_spender(spenders, "legacy/pk-wrongkey", hashtype=hashtype, p2sh=p2sh, witv0=witv0, standard=standard, script=CScript([pubkey1, OP_CHECKSIG]), **SINGLE_SIG, key=eckey1, failure={"key": eckey2}, sigops_weight=4-3*witv0, **(witv0 and ERR_NULLFAIL or ERR_NO_SUCCESS))
...
class TaprootTest(FreicoinTestFramework):
    ...
    def set_test_params(self):
        ...
        self.extra_args = [["-par=1", "-vbparams=taproot:1:1"], ["-par=1", "-vbparams=taproot:-1:0"]]
