...
class MiniWallet:
    ...
    def create_self_transfer(self, *, fee_rate=Decimal("0.003"), fee=Decimal("0"), utxo_to_spend=None, locktime=0, sequence=0, target_weight=0):
        ...
        if self._mode == MiniWalletMode.RAW_P2PK:
            ...
            tx.vin[0].scriptSig = CScript([OP_NOP] * 12)  # pad to identical size
        elif self._mode == MiniWalletMode.ADDRESS_OP_TRUE:
            tx.wit.vtxinwit = [CTxInWitness()]
            tx.wit.vtxinwit[0].scriptWitness.stack = [bytes([0]) + CScript([OP_TRUE]), b'']
        else:
            assert False
        ...
        if target_weight:
            self._bulk_tx(tx, target_weight-16)
