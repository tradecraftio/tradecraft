...
"""Freicoin P2P network half-a-node.
...
class CBlock(CBlockHeader):
    ...
    def calc_witness_merkle_root(self):
        ...
        # its witness or the 32-byte witness root commitment
        tx = self.vtx[0].serialize_without_witness()
...
class NodeConnCB(object):
    """Callback and helper functions for P2P connection to a freicoind node.
...
class NodeConn(asyncore.dispatcher):
    ...
    def __init__(self, dstaddr, dstport, rpc, callback, net="regtest", services=NODE_NETWORK, send_version=True):
        ...
        logger.info('Connecting to Freicoin Node: %s:%d' % (self.dstaddr, self.dstport))
