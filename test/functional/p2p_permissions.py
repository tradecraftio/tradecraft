...
class P2PPermissionsTests(FreicoinTestFramework):
    ...
    def check_tx_relay(self):
        block_op_true = self.nodes[0].getblock(self.generatetoaddress(self.nodes[0], 100, ADDRESS_FCRT1_P2WSH_OP_TRUE)[0])

        self.log.debug("Create a connection from a forcerelay peer that rebroadcasts raw txs")
        # A test framework p2p connection is needed to send the raw transaction directly. If a full node was used, it could only
        # rebroadcast via the inv-getdata mechanism. However, even for forcerelay connections, a full node would
        # currently not request a txid that is already in the mempool.
        self.restart_node(1, extra_args=["-mempoolfullrbf=0", "-whitelist=forcerelay@127.0.0.1"])
    ...
    def replaceinconfig(self, nodeid, old, new):
        with open(self.nodes[nodeid].freicoinconf, encoding="utf8") as f:
            newText = f.read().replace(old, new)
        with open(self.nodes[nodeid].freicoinconf, 'w', encoding="utf8") as f:
            f.write(newText)
