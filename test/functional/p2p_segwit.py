...
def get_p2pk_script(pubkey):
    """Get the script associated with a P2PK."""
    return CScript([pubkey, CScriptOp(OP_CHECKSIG)])
...
def sign_p2pk_witness_input(script, tx_to, in_idx, hashtype, value, refheight, key):
    ...
    tx_hash = SegwitV0SignatureHash(script[1:], tx_to, in_idx, hashtype, value, refheight)
    signature = key.sign_ecdsa(tx_hash) + chr(hashtype).encode('latin-1')
    tx_to.wit.vtxinwit[in_idx].scriptWitness.stack = [signature, script, b'']
    tx_to.rehash()
...
class SegWitTest(FreicoinTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [
            ["-acceptnonstdtxn=1", "-segwitheight={}".format(SEGWIT_HEIGHT), "-vbparams=auxpow:0:0", "-whitelist=noban@127.0.0.1"],
            ["-acceptnonstdtxn=0", "-segwitheight={}".format(SEGWIT_HEIGHT), "-vbparams=auxpow:0:0"],
            ["-acceptnonstdtxn=1", "-segwitheight=-1", "-vbparams=auxpow:0:0"],
        ]
    ...
    def test_v0_outputs_arent_spendable(self):
        ...
        # Create a p2wsh output
        witness_program = CScript([OP_TRUE])
        witness_hash = hash256(witness_program)
    ...
    def test_standardness_v0(self):
        ...
        script_true = CScript([OP_TRUE])
        witness_program = bytes([0]) + script_true
        witness_hash = hash256(witness_program)
        script_pubkey = CScript([OP_0, witness_hash])

        p2sh_pubkey = hash160(script_true)
        ...
        tx = CTransaction()
        tx.vin = [CTxIn(COutPoint(p2sh_tx.sha256, 0), CScript([script_true]))]
        ...
        tx2.wit.vtxinwit[0].scriptWitness.stack = [witness_program, b'']
        tx2.rehash()
        ...
        tx3.wit.vtxinwit[0].scriptWitness.stack = [witness_program, b'']
        tx3.rehash()
        if not self.segwit_active:
    ...
    def test_p2sh_witness(self):
        ...
        witness_program = CScript([0] + [OP_DROP, OP_TRUE])
        witness_hash = hash256(witness_program)
        ...
        # segwit activation.  Note that older freicoind's that are not
    ...
    def test_witness_commitments(self):
        ...
        witness_program = CScript([0] + [OP_TRUE])
        witness_hash = hash256(witness_program)
        script_pubkey = CScript([OP_0, witness_hash])
        tx.vout.append(CTxOut(self.utxo[0].nValue - 1000, script_pubkey))
        tx.rehash()
    ...
    def test_witness_block_size(self):
        ...
        witness_program = CScript([OP_0] + [OP_2DROP] * NUM_DROPS + [OP_TRUE])
        witness_hash = uint256_from_str(hash256(witness_program))
        ...
        for _ in range(NUM_OUTPUTS):
            ...
            child_tx.wit.vtxinwit[-1].scriptWitness.stack = [b'a' * 195] * (2 * NUM_DROPS) + [witness_program, b'']
        child_tx.rehash()
        self.update_witness_block_with_transactions(block, [parent_tx, child_tx])

        weight = get_block_weight(block)
        additional_bytes = MAX_BLOCK_BASE_SIZE * 4 - weight + 1
        i = 0
    ...
    def test_extra_witness_data(self):
        ...
        witness_program = CScript([OP_0] + [OP_DROP, OP_TRUE])
        witness_hash = hash256(witness_program)
        script_pubkey = CScript([OP_0, witness_hash])
        ...
        tx2.wit.vtxinwit[0].scriptWitness.stack = [CScript([CScriptNum(1)]), CScript([CScriptNum(1)]), witness_program, b'']
    ...
    def test_max_witness_elements(self):
        """Test that witness stack can only allow up to 32767 byte pushes."""
        ...
        tx2.wit.vtxinwit[0].scriptWitness.stack = [b'a'] * (MAX_WITNESS_ELEMENTS + 1) + [witness_program, b'']
        tx2.rehash()
    ...
    def test_max_witness_program_length(self):
        ...
        # This program is 19 max pushes (9937 bytes), then 63 more opcode-bytes.
        long_witness_program = CScript([OP_0] + [b'a' * MAX_SCRIPT_ELEMENT_SIZE] * 19 + [OP_DROP] * 62 + [OP_TRUE])
        assert_equal(len(long_witness_program), MAX_PROGRAM_LENGTH + 1)
        long_witness_hash = hash256(long_witness_program)
        ...
        tx2.vout.append(CTxOut(tx.vout[0].nValue - 1000, CScript([OP_TRUE])))
        tx2.wit.vtxinwit.append(CTxInWitness())
        tx2.wit.vtxinwit[0].scriptWitness.stack = [b'a'] * 43 + [long_witness_program, b'']
        tx2.rehash()
        ...
        witness_program = CScript([OP_0] + [b'a' * MAX_SCRIPT_ELEMENT_SIZE] * 19 + [OP_DROP] * 61 + [OP_TRUE])
        ...
        tx2.vout.append(CTxOut(tx.vout[0].nValue - 1000, CScript([OP_TRUE])))
        tx2.wit.vtxinwit.append(CTxInWitness())
        tx2.wit.vtxinwit[0].scriptWitness.stack = [b'a'] * 42 + [witness_program, b'']
        tx2.rehash()
    ...
    def test_witness_input_length(self):
        ...
        for i in range(11):
            tx2.wit.vtxinwit.append(CTxInWitness())
            tx2.wit.vtxinwit[i].scriptWitness.stack = [b'a', witness_program, b'']
        ...
        tx2.wit.vtxinwit.append(CTxInWitness())
        tx2.wit.vtxinwit[-1].scriptWitness.stack = [b'a', witness_program, b'']
        tx2.wit.vtxinwit[5].scriptWitness.stack = [witness_program, b'']
        ...
        tx2.wit.vtxinwit[5].scriptWitness.stack = [b'a', witness_program, b'']
        block.vtx = [block.vtx[0], block.vtx[-1]]
        self.update_witness_block_with_transactions(block, [tx2])
        test_witness_block(self.nodes[0], self.test_node, block, accepted=True)
    ...
    def test_tx_relay_after_segwit_activation(self):
        ...
        witness_program = CScript([OP_0] + [OP_TRUE])
        witness_hash = hash256(witness_program)
        script_pubkey = CScript([OP_0, witness_hash])
        tx2 = CTransaction()
        ...
        witness_program2 = CScript([0] + [b'a' * 400000])
        tx3.vout.append(CTxOut(tx2.vout[0].nValue - 1000, CScript([OP_HASH160, p2sh_pubkey, OP_EQUAL])))
        tx3.wit.vtxinwit[0].scriptWitness.stack = [b''] * 32768 + [witness_program2, b'']
        tx3.rehash()
        ...
        tx3.wit.vtxinwit[0].scriptWitness.stack = [CScript([CScriptNum(1)])] * 32768 + [witness_program, b'']
        tx3.rehash()
    ...
    def test_segwit_versions(self):
        ...
        tx3.wit.vtxinwit[-1].scriptWitness.stack = [witness_program, b'']
    ...
    def test_premature_coinbase_witness_spend(self):
        ...
        spend_tx.vout = [CTxOut(block.vtx[0].vout[0].nValue, CScript([OP_TRUE]))]
        spend_tx.wit.vtxinwit.append(CTxInWitness())
        spend_tx.wit.vtxinwit[0].scriptWitness.stack = [witness_program, b'']
        spend_tx.rehash()
    ...
    def test_uncompressed_pubkey(self):
        ...
        p2pk = CScript([pubkey, CScriptOp(OP_CHECKSIG)])
        witness_program = b'\x00' + p2pk
        witness_long_hash = hash256(witness_program)
        witness_short_hash = ripemd160(witness_long_hash)
        script_wpk = CScript([OP_0, witness_short_hash])
        tx = CTransaction()
        ...
        script_wsh = CScript([OP_0, witness_long_hash])

        tx2 = CTransaction()
        tx2.vin.append(CTxIn(COutPoint(tx.sha256, 0), b""))
        tx2.vout.append(CTxOut(tx.vout[0].nValue - 1000, script_wsh))
        sig_hash = SegwitV0SignatureHash(p2pk, tx2, 0, SIGHASH_ALL, tx.vout[0].nValue, tx.lock_height)
        ...
        tx3.wit.vtxinwit.append(CTxInWitness())
        sign_p2pk_witness_input(witness_program, tx3, 0, SIGHASH_ALL, tx2.vout[0].nValue, tx2.lock_height, key)
        ...
        script_pubkey = get_p2pkh_script(hash160(pubkey))
        tx4 = CTransaction()
    ...
    def test_signature_version_1(self):
        ...
        witness_program = CScript([OP_0] + [pubkey, CScriptOp(OP_CHECKSIG)])
        witness_hash = hash256(witness_program)
        script_pubkey = CScript([OP_0, witness_hash])
        ...
        for sigflag in [0, SIGHASH_ANYONECANPAY]:
            for hashtype in [SIGHASH_ALL, SIGHASH_NONE, SIGHASH_SINGLE]:
                ...
                sign_p2pk_witness_input(witness_program, tx, 0, hashtype, prev_utxo.nValue + 1, tx.lock_height, key)
                self.update_witness_block_with_transactions(block, [tx])
                test_witness_block(self.nodes[0], self.test_node, block, accepted=False)
                ...
                sign_p2pk_witness_input(witness_program, tx, 0, hashtype, prev_utxo.nValue, tx.lock_height, key)
                block.vtx.pop(-2)
                self.update_witness_block_with_transactions(block, [tx])
                test_witness_block(self.nodes[0], self.test_node, block, accepted=True)
        ...
        tx.wit.vtxinwit.append(CTxInWitness())
        sign_p2pk_witness_input(witness_program, tx, 0, SIGHASH_ALL, prev_utxo.nValue, tx.lock_height, key)
        ...
        for i in range(NUM_SIGHASH_TESTS):
            ...
            for i in range(num_inputs):
                ...
                sign_p2pk_witness_input(witness_program, tx, i, hashtype, temp_utxos[i].nValue, 0, key)
        ...
        script_pk = CScript([OP_0, ripemd160(hash256(bytes([0]) + CScript([pubkey, CScriptOp(OP_CHECKSIG)])))])
        tx = CTransaction()
        tx.vin.append(CTxIn(COutPoint(temp_utxos[0].sha256, temp_utxos[0].n), b""))
        tx.vout.append(CTxOut(temp_utxos[0].nValue, script_pk))
        tx.wit.vtxinwit.append(CTxInWitness())
        sign_p2pk_witness_input(witness_program, tx, 0, SIGHASH_ALL, temp_utxos[0].nValue, 0, key)
        tx2 = CTransaction()
        tx2.vin.append(CTxIn(COutPoint(tx.sha256, 0), b""))
        tx2.vout.append(CTxOut(tx.vout[0].nValue, CScript([OP_TRUE])))

        script = get_p2pk_script(pubkey)
        sig_hash = SegwitV0SignatureHash(script, tx2, 0, SIGHASH_ALL, tx.vout[0].nValue, tx.lock_height)
        signature = key.sign_ecdsa(sig_hash) + b'\x01'  # 0x1 is SIGHASH_ALL
        ...
        for i in temp_utxos:
            ...
            tx.wit.vtxinwit.append(CTxInWitness())
            sign_p2pk_witness_input(witness_program, tx, index, SIGHASH_ALL | SIGHASH_ANYONECANPAY, i.nValue, 0, key)
            index += 1
        block = self.build_next_block()
        self.update_witness_block_with_transactions(block, [tx])
        test_witness_block(self.nodes[0], self.test_node, block, accepted=True)
    ...
    def test_non_standard_witness(self):
        ...
        scripts = []
        scripts.append(bytes([0]) + CScript([OP_DROP] * 100))
        scripts.append(bytes([0]) + CScript([OP_DROP] * 99))
        scripts.append(bytes([0]) + CScript([pad * 59] * 59 + [OP_DROP] * 58 + [OP_2DROP]))
        scripts.append(bytes([0]) + CScript([pad * 59] * 59 + [OP_DROP] * 59 + [OP_2DROP]))
        ...
        p2wsh_txs[0].wit.vtxinwit[0].scriptWitness.stack = [pad] * 101 + [scripts[0], b'']
        test_transaction_acceptance(self.nodes[1], self.std_node, p2wsh_txs[0], True, False, 'bad-witness-nonstandard')
        # Non-standard nodes should accept
        test_transaction_acceptance(self.nodes[0], self.test_node, p2wsh_txs[0], True, True)

        # Stack element size over 80 bytes is non-standard
        p2wsh_txs[1].wit.vtxinwit[0].scriptWitness.stack = [pad * 81] * 100 + [scripts[1], b'']
        test_transaction_acceptance(self.nodes[1], self.std_node, p2wsh_txs[1], True, False, 'bad-witness-nonstandard')
        # Non-standard nodes should accept
        test_transaction_acceptance(self.nodes[0], self.test_node, p2wsh_txs[1], True, True)
        # Standard nodes should accept if element size is not over 80 bytes
        p2wsh_txs[1].wit.vtxinwit[0].scriptWitness.stack = [pad * 80] * 100 + [scripts[1], b'']
        test_transaction_acceptance(self.nodes[1], self.std_node, p2wsh_txs[1], True, True)

        # witnessScript size at 3600 bytes is standard
        p2wsh_txs[2].wit.vtxinwit[0].scriptWitness.stack = [pad, pad, scripts[2], b'']
        test_transaction_acceptance(self.nodes[0], self.test_node, p2wsh_txs[2], True, True)
        test_transaction_acceptance(self.nodes[1], self.std_node, p2wsh_txs[2], True, True)

        # witnessScript size at 3601 bytes is non-standard
        p2wsh_txs[3].wit.vtxinwit[0].scriptWitness.stack = [pad, pad, pad, scripts[3], b'']
    ...
    def test_upgrade_after_activation(self):
        ...
        assert softfork_active(self.nodes[2], 'finaltx')
    ...
    def test_witness_sigops(self):
        ...
        witness_program = CScript([0] + [OP_TRUE, OP_IF, OP_TRUE, OP_ELSE] + [OP_CHECKMULTISIG] * 5 + [OP_CHECKSIG] * 193 + [OP_ENDIF])
        witness_hash = hash256(witness_program)
        script_pubkey = CScript([OP_0, witness_hash])
        ...
        witness_program_toomany = CScript([0] + [OP_TRUE, OP_IF, OP_TRUE, OP_ELSE] + [OP_CHECKSIG] * (extra_sigops_available + 1) + [OP_ENDIF])
        witness_hash_toomany = hash256(witness_program_toomany)
        script_pubkey_toomany = CScript([OP_0, witness_hash_toomany])

        # If we spend this script instead, we would exactly reach our sigop
        # limit (for witness sigops).
        witness_program_justright = CScript([0] + [OP_TRUE, OP_IF, OP_TRUE, OP_ELSE] + [OP_CHECKSIG] * (extra_sigops_available) + [OP_ENDIF])
        witness_hash_justright = hash256(witness_program_justright)
        ...
        for i in range(outputs - 1):
            tx2.vin.append(CTxIn(COutPoint(tx.sha256, i), b""))
            tx2.wit.vtxinwit.append(CTxInWitness())
            tx2.wit.vtxinwit[-1].scriptWitness.stack = [witness_program, b'']
            total_value += tx.vout[i].nValue
        tx2.wit.vtxinwit[-1].scriptWitness.stack = [witness_program_toomany, b'']
        ...
        tx2.wit.vtxinwit.append(CTxInWitness())
        tx2.wit.vtxinwit[-1].scriptWitness.stack = [witness_program_justright, b'']
        tx2.rehash()
