...
def sign_p2pk_witness_input(script, tx_to, in_idx, hashtype, value, key):
    ...
    tx_hash = SegwitVersion1SignatureHash(script[1:], tx_to, in_idx, hashtype, value)
...
class SegWitTest(FreicoinTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [
            ["-whitelist=127.0.0.1", "-acceptnonstdtxn=1", "-segwitheight={}".format(SEGWIT_HEIGHT), "-vbparams=auxpow:0:0"],
            ["-whitelist=127.0.0.1", "-acceptnonstdtxn=0", "-segwitheight={}".format(SEGWIT_HEIGHT), "-vbparams=auxpow:0:0"],
            ["-whitelist=127.0.0.1", "-acceptnonstdtxn=1", "-segwitheight=-1", "-vbparams=auxpow:0:0"]
        ]
    ...
    def test_max_witness_program_length(self):
        ...
        long_witness_program = CScript([OP_0] + [b'a' * 520] * 19 + [OP_DROP] * 62 + [OP_TRUE])
        ...
        witness_program = CScript([OP_0] + [b'a' * 520] * 19 + [OP_DROP] * 61 + [OP_TRUE])
    ...
    def test_uncompressed_pubkey(self):
        ...
        sig_hash = SegwitVersion1SignatureHash(p2pk, tx2, 0, SIGHASH_ALL, tx.vout[0].nValue)
        ...
        # Should fail policy test.
        test_transaction_acceptance(self.nodes[0], self.test_node, tx4, True, False, 'non-mandatory-script-verify-flag (Extra items left on stack after execution)')
        block = self.build_next_block()
