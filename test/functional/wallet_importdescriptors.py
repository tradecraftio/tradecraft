...
class ImportDescriptorsTest(FreicoinTestFramework):
    ...
    def run_test(self):
        ...
        for i, expected_addr in enumerate(addresses):
            ...
            assert_equal(bech32_addr_info['desc'][:23], 'wpk([80002067/0\'/0\'/{}]0'.format(i))

            shwpk_addr = w1.getnewaddress('', 'p2sh-segwit')
            shwpk_addr_info = w1.getaddressinfo(shwpk_addr)
            assert_equal(shwpk_addr_info['desc'][:26], 'sh(wpk([abcdef12/0\'/0\'/{}]0'.format(i))
