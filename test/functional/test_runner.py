...
def main():
    ...
    tmpdir = "%s/freicoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_freicoind):
        print("No functional tests to run. Wallet, utils, and freicoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[]):
    # Warn if freicoind is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "freicoind"]) is not None:
    ...
    if "FREICOIND" not in os.environ:
        os.environ["FREICOIND"] = build_dir + '/src/freicoind' + exeext
        os.environ["FREICOINCLI"] = build_dir + '/src/freicoin-cli' + exeext
