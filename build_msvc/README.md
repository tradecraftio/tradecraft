...
msbuild /m build_msvc\freicoin.sln /p:Platform=x64 /p:Configuration=Release /t:build
```
...
The [external dependencies](https://github.com/tradecraftio/tradecraft/blob/master/doc/dependencies.md) required for building are:
...
Qt
---------------------
All the Freicoin applications are configured to build with static linking. In order to build the Freicoin Qt applications a static build of Qt is required.

The runtime library version (e.g. v141, v142) and platform type (x86 or x64) must also match. OpenSSL must also be linked into the Qt binaries in order to provide full functionality of the Freicoin Qt programs. An example of the configure command to build Qtv5.9.7 locally to link with Freicoin is shown below (adjust paths accordingly), note it can be expected that the configure and subsequent build will fail numerous times until dependency issues are resolved.
