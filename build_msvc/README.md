...
Introduction
---------------------
Visual Studio 2022 is minimum required to build Freicoin.

Solution and project files to build with `msbuild` or Visual Studio can be found in the `build_msvc` directory.

To build Freicoin from the command-line, it is sufficient to only install the [Visual Studio Build Tools](https://visualstudio.microsoft.com/downloads/) component.
...
```cmd
msbuild build_msvc\freicoin.sln -property:Configuration=Release -maxCpuCount -verbosity:minimal
```
